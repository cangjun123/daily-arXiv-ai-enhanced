...
TL;DR: 提出了一种在RGB空间使用五进制像素强度组合将文本数据嵌入图像的新技术，通过单像素实现高效编码，避免传统方法的多像素需求和高计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有文本隐藏方法（如LSB、PVD、变换域方法、深度学习等）大多依赖多像素操作或变换系数，编码确定性强、计算开销大，且可能引入噪声。需要一种更高效、低失真的单像素级嵌入方案。

Method: 利用RGB通道中各五个受控的像素强度变化，形成125种独特的像素强度组合，每种组合映射到一个文本符号（字母、数字、空格、特殊字符），从而实现单个RGB像素编码一个完整字符。

Result: 经MSE、MAE、SNR、PSNR、SSIM、直方图比较和热图分析等多种指标评估，原始图像与编码图像无显著失真，嵌入效率优于传统方法。

Conclusion: 该方法通过五进制像素强度组合实现了高效、低失真的文本嵌入，单像素编码能力超越了需多像素或高计算成本的现有技术，为图像隐写提供了新思路。
...
TL;DR: ReHyAt提出了一种结合软注意力和线性注意力优点的循环混合注意力机制，将注意力计算复杂度从二次降至线性，大幅降低了长序列视频生成的训练成本


<details>
  <summary>Details</summary>
Motivation: 现有基于transformer的视频扩散模型虽然能生成高质量视频，但其二次注意力复杂度限制了长序列的可扩展性，迫切需要更高效的注意力机制

Method: 使用循环混合注意力机制，结合软注意力的准确性和线性注意力的效率，通过分块循环重构实现恒定内存使用，并设计轻量级蒸馏和微调流程

Result: 在VBench和VBench-2.0基准测试上达到SOTA视频质量，注意力计算成本从二次降至线性，训练成本降低两个数量级至约160GPU小时

Conclusion: ReHyAt为双向软注意力模型提供了实用的可扩展解决方案，解锁了长时长和设备端视频生成的可行性
...
TL;DR: 对比研究发现，预训练模型的迁移学习在图像分类任务上优于自定义CNN和特征提取方法，特别是在数据有限时效果更为显著。


<details>
  <summary>Details</summary>
Motivation: 对比自定义CNN与主流预训练架构在处理孟加拉国多样图像分类任务时的表现，为实践者根据数据特性、计算资源和性能需求选择合适方法提供参考。

Method: 对比自定义CNN、ResNet-18和VGG-16等模型，采用特征提取和迁移学习两种方法，在5个孟加拉国图像数据源进行测试评估。

Result: 迁移学习方法表现最佳，精度提高显著，最高在道路损伤检测数据达到100%准确率。自定义CNN在模型规模和计算效率上有优势，但整体性能相对较弱。

Conclusion: 迁移学习特别适合复杂且小样本分类问题，建议实践者根据场景特点和资源限制在精度和效率之间权衡选择。对于数据有限、计算资源相对充足的任务，推荐使用预训练模型的迁移学习方法。
...
TL;DR: PackCache：用于统一自回归视频生成的训练免费KV缓存管理方法，通过动态压缩KV缓存加速长序列生成


<details>
  <summary>Details</summary>
Motivation: 统一自回归模型虽然能处理多模态任务，但KV缓存随生成token数线性增长，成为推理效率和生成长度的瓶颈。视频生成尤其受此限制，需要减少KV缓存负担以支持更长序列生成。

Method: 提出了PackCache方法，利用KV缓存token的时空属性：文本和条件图像token作为持久语义锚点持续获得高注意力；注意力随时间距离自然衰减。通过三个协调机制动态压缩KV缓存：1)条件锚定保留语义参考；2)跨帧衰减建模按时间距离分配缓存预算；3)空间保留位置嵌入在缓存移除时保持一致的3D结构。

Result: 在48帧长序列上，PackCache将端到端生成加速1.7-2.2倍。在受KV缓存扩展影响最大的最后四帧（视频中最昂贵的部分），PackCache在A40和H200上分别实现2.6倍和3.7倍加速。

Conclusion: PackCache是一种有效的训练免费KV缓存管理方法，能够显著提升自回归视频生成的效率，特别适用于长序列生成，展示了实现更长序列视频生成的潜力。
...
TL;DR: 本文提出使用EMOCA导出的3D表情和姿态系数分析分心驾驶中的压力状态，构建Transformer时间建模框架评估跨模态注意力融合策略，在压力识别任务上取得优异性能


<details>
  <summary>Details</summary>
Motivation: 当前基于面部视频的压力识别方法主要依赖面部动作单元，而解耦的3D面部几何特征作用未被充分探索。压力识别面临主观性和自主面部控制的挑战，需要更可靠的分析方法

Method: 1) 使用EMOCA提取3D表情和姿态系数分析基线期和压力期的差异；2) 配对假设检验揭示41个系数显示特定的压力响应；3) 提出基于Transformer的时间建模框架，评估单模态、早期融合和跨模态注意力三种融合策略

Result: 1) 配对检验显示56个系数中41个呈现一致且阶段特定的压力响应；2) EMOCA与生理信号的跨模态注意力融合表现最佳(AUROC 92%，准确率86.7%)；3) EMOCA与注视融合也表现良好(AUROC 91.8%)

Conclusion: 3D面部几何系数能有效反映压力响应，时间建模和跨模态注意力策略显著提升压力识别性能，为可靠的视频压力检测提供新方法
...
TL;DR: 研究表明，通过少量配对图像进行LoRA调优的流匹配基础模型可以实现RGB到非可见光谱（红外和合成孔径雷达）的跨模态翻译，并能有效提升下游目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 可见光基础模型主要基于RGB数据训练，而许多安全关键应用依赖红外和合成孔径雷达等非可见光谱模态。研究者探索是否能用少量配对数据将RGB预训练模型重新用于跨光谱翻译，并验证生成数据对下游检测的增强效果。

Method: 基于FLUX.1 Kontext模型，插入低秩适应模块，仅用每个域100张配对图像进行微调，实现RGB到红外和SAR的像素对齐转换。通过LPIPS指标评估翻译质量，并用于选择最佳LoRA适配器。生成的数据用于训练纯目标模态的检测模型。

Result: 在KAIST红外数据集和M4-SAR数据集上，较低LPIPS值始终预测较高的检测mAP。使用最佳LPIPS选择适配器生成的合成红外和SAR数据，结合有限真实数据能显著提升行人检测和基础设施检测性能。

Conclusion: 少量样本的LoRA适配流匹配基础模型是实现非可见光谱模态基础模型支持的有效途径，为跨模态应用提供了实用解决方案。
...
TL;DR: 本文提出了一个混合自监督与弱监督学习的框架，用于从术前CT扫描中预测乳突切除术区域，无需人工标注，为人工耳蜗手术规划提供了新解决方案。


<details>
  <summary>Details</summary>
Motivation: 乳突切除术是人工耳蜗手术的重要步骤，通过术前影像准确预测其形状可以改善手术规划、降低风险并提高手术效果。由于获取真实标注数据存在挑战，目前缺乏基于深度学习的研究，本文旨在填补这一空白。

Method: 提出混合自监督和弱监督学习框架，直接从术前完整乳突的CT扫描中预测乳突切除术区域，采用3D T-分布损失函数处理弱监督医学影像任务。

Result: 该方法在预测复杂且无明确边界的乳突切除术形状时达到平均Dice得分0.72，超越了现有最佳方法，表现出色。

Conclusion: 这是首项将自监督与弱监督学习结合用于乳突切除术形状预测的研究，提供了直接从术前CT扫描构建3D术后乳突切除术表面的基础，为人耳蜗手术规划提供了稳健高效的解决方案。
...
TL;DR: 提出了CRUNet-MR-Univ基础模型，通过利用时空相关性和提示先验来应对多样的心脏MRI重建任务，展现出优于基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习在心脏MRI重建中表现出色，但现有模型在处理多样化的CMR扫描变异性（包括图像对比度、采样模式、扫描仪厂商、解剖结构和疾病类型等）时泛化能力受限，需要开发能够适应广泛CMR场景的统一模型。

Method: 提出了CRUNet-MR-Univ基础模型，通过利用时空相关性和基于提示的先验知识，来处理CMR扫描的完整多样性。

Result: 该方法在广泛的设置中一致优于基线方法，展示了其有效性和前景。

Conclusion: CRUNet-MR-Univ基础模型能够有效处理CMR扫描的多样性，为心脏MRI重建提供了具有良好泛化能力的统一解决方案。
...
TL;DR: 本文针对大型视觉语言模型在链式思维推理中产生的过思考问题，提出了GPRO优化框架，通过动态路由计算路径来平衡视觉感知与推理决策，显著提升了准确性与效率。


<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉语言模型采用链式思维机制进行逐步推理，但这种方法容易导致过思考现象——即使是简单查询也会产生冗长响应，不仅降低测试效率，有时还会损害准确性。先前的研究通过自适应推理策略尝试缓解此问题，但忽略了关键瓶颈：视觉感知失败。我们认为稳定的推理依赖于低层视觉基础，推理错误往往源自不完美的感知而非推理不足。

Method: 提出了GPRO（门控感知-推理优化）元推理控制器，该控制器在每个生成步骤动态路由计算到三条决策路径：轻量快速路径、用于重新检查视觉输入的慢速感知路径，以及用于内部自省的慢速推理路径。为学习这种区分，我们从约79万个样本中获取大规模故障归因监督，使用教师模型区分感知幻觉与推理错误。然后通过多目标强化学习训练控制器，在不确定性下优化任务准确性与计算成本之间的权衡。

Result: 在五个基准测试上的实验表明，GPRO显著提高了准确性和效率，优于最近的慢思考方法，同时生成更短的响应。

Conclusion: GPRO框架通过动态路由机制有效解决了大型视觉语言模型中的过思考问题，证明了对感知误差和推理过程同时优化的必要性，为视觉语言模型的效率与准确性平衡提供了新的解决方案。
...
TL;DR: FaceRefiner提出了一种基于风格迁移的人脸纹理优化方法，通过多层次信息传递，在保持人脸细节、结构和ID一致性的同时，提升纹理质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在合成UV纹理时，常因训练数据或人脸生成器空间的限制，导致纹理细节、结构和身份信息与输入图像不一致。作者旨在通过风格迁移技术实现更忠实于原始输入的纹理优化。

Method: FaceRefiner把3D采样的纹理视为“风格”，将纹理生成方法的输出视为“内容”。通过融合可微分渲染技术，实现了高、中、低多层次信息的风格迁移，尤其在可见的人脸区域能够保留像素级别的细节。

Result: 在Multi-PIE、CelebA和FFHQ数据集上进行的广泛实验表明，该方法在纹理质量和人脸身份保持能力上超越了现有先进方法。

Conclusion: FaceRefiner是一种有效的人脸纹理优化方法，通过多层次风格迁移技术，实现了更好的细节、结构和身份信息保持能力，提高了在真实场景输入下的生成纹理质量。
...
TL;DR: MiLDEAgent是多层设计文档编辑的推理框架，结合了RL训练的多模态推理器和图像编辑器，在20K设计文档基准上显著优于开源模型，性能接近闭源模型。


<details>
  <summary>Details</summary>
Motivation: 真实设计文档是多层次结构，包含装饰、文本和图像层，而现有工作忽视多层文档编辑，主要关注单层图像编辑或多层生成，缺乏确定修改哪些层以及如何协调修改的细粒度推理能力。

Method: 提出MiLDEAgent框架：1) 采用RL训练的多模态推理器进行层次感知理解；2) 结合图像编辑器进行针对性修改；同时构建MiLDEBench基准(20K设计文档+多样化编辑指令)和MiLDEEval四维度评估协议。

Result: 在14个开源和2个闭源模型的广泛实验中，现有方法泛化能力差：开源模型常无法完成多层文档编辑任务，闭源模型存在格式违规问题；MiLDEAgent实现强层感知推理和精确编辑，显著优于所有开源基线，性能与闭源模型相当。

Conclusion: MiLDEAgent建立了多层文档编辑的第一个强基线，通过分层推理和精准编辑能力填补了该领域的空白，为真实设计文档的自然语言指令编辑提供了有效解决方案。
...
TL;DR: 论文讨论了AI赋能的以人为中心的CPS在不确定条件下可能出现的安全和安保风险，并提出了一个评估各类防护策略的框架，以餐前未通知检测在闭环血糖控制中的应用为例。


<details>
  <summary>Details</summary>
Motivation: 以人为中心的CPS（如医疗监控系统、自动驾驶汽车等）在运行中可能因与人类交互等原因进入不确定状态，这将违反系统的安全和安保要求。需要系统性地评估如何保障此类系统在部署中的安全性。

Method: 首先讨论可能导致系统进入未知条件的操作偏差；其次创建一个框架来评估不同的安全和安保策略；最后以1型糖尿病患者闭环血糖控制中未通知进餐的检测为例，展示一种基于图像的个性化技术。

Result: 论文提出了一个评估AI赋能CPS安全策略的框架，并通过具体案例（血糖控制中的进餐检测）展示了该框架的应用。

Conclusion: AI赋能的以人为中心的CPS在部署中面临因操作偏差导致的不确定条件风险，需要一个系统性的框架来评估和确保其安全与安保，论文提出的框架及案例为此提供了解决方案。
...
TL;DR: 该论文提出了一种高不确定性区域引导的多架构协同学习(HUR-MACL)模型，用于头颈部多器官分割。通过CNN自适应识别高不确定性区域，在关键区域结合Vision Mamba和Deformable CNN联合提升分割精度，实现SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 头颈部危机器官分割对放射治疗至关重要，但深度学习模型在处理小而复杂形状器官时常失败。现有混合架构只是简单拼接特征，未能充分利用各组件独特优势，导致功能重叠且分割精度有限。

Method: 提出HUR-MACL模型：1)使用CNN自适应识别高不确定性区域；2)在高不确定性区域联合运用Vision Mamba和Deformable CNN提升分割精度；3)引入异构特征蒸馏损失促进两个架构在关键区域的协同学习。

Result: 在两个公开数据集和一个私有数据集上均取得了最先进的(SOTA)结果。

Conclusion: HUR-MACL模型通过针对性处理高不确定性区域，有效结合不同模型架构的优势，显著提升了头颈部多器官分割的准确性。该方法为解决小而复杂形状器官的分割问题提供了有效方案。
...
TL;DR: 本文提出HyperAlign：基于双曲蕴涵几何的文本-图像对齐评估框架，通过将CLIP特征映射到双曲空间、设计动态监督蕴涵建模机制，并使用自适应调制回归器来校准余弦相似度，在多个任务上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成技术快速发展，但现有评估方法依赖于欧几里得空间度量，忽略了语义对齐的结构化特性，且缺乏对不同样本的自适应能力。

Method: 1）使用CLIP提取欧几里得特征并映射到双曲空间；2）设计动态监督蕴涵建模机制，将离散蕴涵逻辑转化为连续几何结构监督；3）提出自适应调制回归器，利用双曲几何特征生成样本级调制参数，自适应校准欧几里得余弦相似度来预测最终得分。

Result: HyperAlign在单数据库评估和跨数据库泛化任务上都取得了极具竞争力的性能，充分验证了双曲几何建模对图像-文本对齐评估的有效性。

Conclusion: 基于双曲蕴涵几何的框架能有效解决文本-图像对齐评估问题，通过结构化语义建模和自适应校准机制，显著提升评估性能。
...
TL;DR: Agri-R1是用于农业领域疾病诊断的增强推理大模型，通过自动生成高质量推理数据和使用GRPO训练，在参数较少的情况下取得了优于参数更大的基准模型的性能表现。


<details>
  <summary>Details</summary>
Motivation: 农业疾病诊断对视觉语言模型提出挑战，传统微调需要大量标注、缺乏可解释性、泛化能力差。现有推理方法依赖昂贵的专家标注，且难以处理农业领域开放多样的查询。

Method: 1）通过视觉语言合成和基于LLM的过滤自动生成高质量推理数据，仅需19%可用样本；2）使用Group Relative Policy Optimization（GRPO）训练，创新性地结合领域特定词典和模糊匹配的奖励函数来评估开放回答的正确性和语言灵活性。

Result: 3B参数模型在CDDMBench上表现优于7B-13B参数基线：疾病识别准确率相对提升23.2%，农业知识问答提升33.3%，跨领域泛化能力比标准微调提高26.10分。消融研究证实结构化推理数据与GRPO驱动的探索协同作用支持这些改进，且随着问题复杂度增加效果更明显。

Conclusion: Agri-R1通过自动化数据生成和创新的GRPO训练方法，显著提升了农业疾病诊断模型的性能，在参数效率、准确性和泛化能力方面都有明显改进。
...
TL;DR: HuForDet是一种综合的人像伪造检测框架，通过双分支架构（面部伪造检测分支和全身体上下文伪造检测分支）实现对各种人像伪造类型的通用检测。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成内容（AIGC）快速发展，深度伪造威胁日益严重，但现有检测方法零散且专门化，要么专注于面部区域伪造，要么专注于全身合成图像，无法通用地检测完整的人像伪造谱系。

Method: 1)面部伪造检测分支：在RGB和频域使用异构专家，包含自适应拉普拉斯-高斯（LoG）模块捕捉从细粒度融合边界到粗粒度纹理异常的伪影。2)上下文伪造检测分支：利用多模态大语言模型（MLLM）分析全身语义一致性，配备置信度估计机制，在特征融合时动态加权其贡献。同时创建了统一的人像伪造数据集（HuFor），结合现有面部伪造数据和新收集的全身合成人像数据。

Result: 大量实验表明，HuForDet在各种人像伪造检测上实现了最先进的性能，并展现出卓越的鲁棒性。

Conclusion: HuForDet提供了一个整体性的人像伪造检测框架，能够统一处理面部和全身伪造，在多样化的伪造类型上表现出优异和稳健的检测能力。
...
TL;DR: ProFuse框架实现开放词汇3D场景理解，通过3D高斯泼溅在直接注册过程中提升跨视角一致性和遮罩内聚性，无需渲染监督微调，语义标注速度比当前最佳方法快2倍


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法在开放词汇场景理解中面临跨视角不一致、遮罩碎片化问题，且现有方法通常依赖预训练模型，计算开销大、效率低

Method: 引入3D上下文提议构建机制，通过跨视角聚类建立密集对应关系引导的预注册阶段，使用加权聚合的全局特征与高斯泼溅融合，实现无需额外优化的语义关联

Result: 在保持几何精炼的同时，每个场景约5分钟完成语义标注，速度比当前最佳方法快2倍，实现强开放的3D高斯泼溅理解

Conclusion: ProFuse提供高效的开放词汇3D场景理解框架，通过创新的上下文提议和特征融合机制，在最小化计算开销的同时确保跨视角语义一致性
...
TL;DR: 该研究提出了一种增强版Pix2Pix框架，结合SEResNet和U-Net++，在少样本条件下显著提升MRI图像翻译的质量和结构保真度。


<details>
  <summary>Details</summary>
Motivation: MRI成像存在采集时间长、成本高和分辨率受限等问题，限制了临床应用。尽管Pix2Pix在医学图像翻译中广泛应用，但其潜力尚未充分挖掘，特别是在保持结构保真度和少样本学习方面需要改进。

Method: 提出增强的Pix2Pix框架：1）集成SEResNet，通过通道注意力机制增强关键特征表示；2）采用U-Net++提升多尺度特征融合；3）使用简化的PatchGAN判别器稳定训练并优化局部解剖真实性。

Result: 在少于500张图像的少样本条件下，该方法在多个MRI模态内翻译任务中均实现了稳定的结构保真度和优异的图像质量，表现出强大的泛化能力。

Conclusion: 该方法有效扩展了Pix2Pix在医学图像翻译中的应用，为MRI图像生成提供了一种高质量的少样本解决方案。
...
TL;DR: 我们介绍了MCLC，一种基于理论、即插即用的纠正模块，通过测量一致的Langevin更新来解决基于LDM的逆求解器的不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 尽管潜在扩散模型(LDMs)作为领域无关的零样本逆求解器被广泛研究，但现有求解器仍存在不稳定性，导致伪影和图像质量下降。本文旨在识别并解决这些不稳定性的根源。

Method: 我们首先识别不稳定性源于求解器与真实反向扩散动力学之间的差异，然后提出MCLC模块，通过测量一致的Langevin更新来减小这种差距，该模块无需线性流形假设，可即插即用地改进现有求解器。

Result: 实验证明MCLC在多种图像恢复任务中有效提升稳定性，与现有求解器兼容良好，同时分析了blob伪影的成因并提供了解决方案。

Conclusion: MCLC是构建更稳定的零样本逆问题求解器的关键步骤，其理论基础和实际效果展示了其在扩散模型逆求解器中的潜力。
...
TL;DR: 本文提出了一种将预训练扩散模型通过低成本微调转换为金字塔模型的管道，并在保持视频质量的同时提高了推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有的开源金字塔视频模型需要从头训练，且在视觉合理性上不如最先进的系统，同时现有方法无法充分利用已训练的扩散模型。

Method: 开发了将预训练扩散模型转换为金字塔模型的微调管道，并研究了金字塔模型内部的步骤蒸馏策略以进一步提升推理效率。

Result: 该方法在保持输出视频质量的同时，成功将预训练模型转换为金字塔模型，并提高了推理效率。

Conclusion: 该研究证明了通过低成本的微调可以将预训练扩散模型有效转换为金字塔模型，同时探索了步骤蒸馏等优化策略，为高效视频生成提供了可行的解决方案。
...
TL;DR: 评估SAMURAI基础模型在无人机跟踪中的潜力并提出检测器增强的扩展方法，在复杂城市环境中显著提升跟踪鲁棒性，尤其是在长持续时间序列和无人机离开再进入场景中。


<details>
  <summary>Details</summary>
Motivation: 现代监控系统需要鲁棒的长期无人机跟踪，而现有的RGB跟踪研究有限且依赖传统运动模型。尽管SAMURAI等基础模型在其他领域展现出强大的类别无关跟踪性能，但在无人机特定场景中的应用尚未被研究。

Method: 首先系统评估SAMURAI在无人机跟踪中的潜力，然后提出检测器增强的SAMURAI扩展方法，通过融合检测器信息缓解边界框初始化和序列长度敏感性。

Result: 提出的扩展方法在复杂城市环境中显著提升了跟踪鲁棒性，在长时间序列和无人机离开再进入事件中效果尤为明显。与SAMURAI的零样本性能相比，在数据集和指标上获得一致提升，成功率最高提升+0.393，误报率最高降低-0.475。

Conclusion: 该研究首次系统评估了SAMURAI在无人机跟踪中的应用潜力，提出的检测器增强扩展显著改善了在复杂城市监控环境中的跟踪性能，为无人机跟踪提供了新解决方案。
...
TL;DR: 提出了基于二值化和互补预处理技术的古文字图像增强方法，用于去除污渍和增强模糊文字，在石碑、金属板和历史文献等多种古文字上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 古文字图像常存在严重背景噪声、低对比度以及老化环境导致的退化问题，前景文字与背景视觉特征相似导致难以辨识，需要提升这类退化古图像的可读性。

Method: 采用基于二值化的图像增强方法，结合互补预处理技术去除污渍并增强模糊不清的古文字。

Result: 使用K-NN分类器在石碑、金属板、文档文字上的分类准确率分别为55.7%、62%、65.6%；使用SVM分类器分别为53.2%、59.5%、67.8%，证明了该方法在提升古马拉地文铭文图像可读性方面的有效性。

Conclusion: 所提出的图像增强方法能有效改善古文字图像的可读性，特别是在处理多种材质上的古文字时表现出良好效果，为古文字学研究提供技术支持。
...
TL;DR: 本文提出了一个面向制药领域的工业级多模态框架，分析了超过40个视觉语言模型在处理长视频时的实际性能、效率瓶颈和部署限制。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型评测大多关注短视频且假设无限计算资源，但在制药等工业场景中需要在GPU、延迟和成本严格限制下处理长格式视频，现有方法难以扩展。

Method: 在工业环境中部署多模态推理框架，处理20万+PDF、2.5万+多格式视频和888个多语言音频；对40多个VLM在两个主流基准和专有数据集上进行实证分析。

Result: 在商用GPU上使用SDPA注意力机制实现3-8倍效率提升；多模态在8/12任务领域提升性能（尤其是长度依赖任务）；在时间对齐和关键帧检测方面发现明显瓶颈。

Conclusion: 本文没有提出新模型，而是揭示了当前VLM在现实部署约束下的实际限制、权衡和失败模式，为工业长视频理解系统的设计提供实践指导。
...
TL;DR: 本文针对卫星图像时间序列农作物分类中，模型对输入序列长度变化适应能力差的问题，提出了一种时域自适应分割方法TEA。该方法通过教师-学生知识蒸馏、原型学习、软标签指导和全序列重构等技术，显著提升了模型在不同时间长度输入下的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有卫星图像时间序列分割方法主要针对固定长度的序列设计，忽视了模型在不同序列长度场景下的泛化能力，导致在序列长度变化时分割性能显著下降。这限制了实际农业应用场景中对不同时长卫星数据的处理效果。

Method: 提出TEA方法：1）设计教师模型编码全局序列知识，指导学生模型适应不同长度的时域输入；2）通过中间嵌入、原型和软标签三个视角进行特征空间指导，实现知识迁移；3）动态聚合学生模型以缓解知识遗忘；4）引入全序列重构作为辅助任务，提升不同长度输入的表示质量。

Result: 该方法在多个常用基准测试中，对不同时间长度输入都带来了显著性能提升，验证了其有效性。

Conclusion: 本文提出的时域自适应分割方法TEA有效解决了卫星图像时间序列分割中模型对序列长度变化敏感的问题，通过知识蒸馏和表示学习技术提高了模型的鲁棒性，为农业遥感应用提供了更实用的解决方案。
...
TL;DR: 提出SparseLaneSTP稀疏注意力transformer方法，整合车道几何先验和时间信息，并创建高质量3D车道数据集，在多个基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D车道检测方法面临几个关键挑战：1）基于鸟瞰图（BEV）的稠密特征检测方法存在特征转换误差，导致特征表示与真实3D路面不对齐；2）尽管稀疏车道检测器超越了稠密BEV方法，但完全忽略了有价值的车道先验知识；3）现有方法未能有效利用历史车道观测信息，而这些信息有助于在能见度差的情况下解决歧义问题。

Method: 提出了SparseLaneSTP方法，其核心创新包括：1）将车道几何结构特性和时间信息集成到稀疏车道transformer中；2）设计了新的车道特定时空注意力机制；3）为稀疏架构定制了连续车道表示；4）引入了时间正则化。此外，针对现有3D车道数据集的不足，作者还使用简单有效的自动标注策略创建了一个精确一致的3D车道数据集。

Result: 实验证明该方法在所有检测和误差指标上都表现出最佳性能，在现有3D车道检测基准和作者提出的新数据集上均达到了最先进水平。

Conclusion: SparseLaneSTP通过整合车道先验知识和时间信息，显著提升了3D车道检测的准确性和鲁棒性，同时创建的高质量数据集为未来研究提供了更好的基准。
...
TL;DR: OceanSplat提出了一种基于3D高斯泼溅的技术，用于解决水下场景中因光学退化导致的多视角不一致问题，通过三目一致性约束和合成深度先验优化几何重建，显著减少漂浮伪影。


<details>
  <summary>Details</summary>
Motivation: 水下场景存在光学散射和吸收，导致传统多视角重建方法产生几何不一致和漂浮伪影，需要专门的技术来分离介质和物体几何。

Method: 1. 通过水平垂直相机视角平移和逆扭曲实现三目视图一致性对齐；2. 利用平移视图三角测量生成合成对极深度先验作为自监督正则器；3. 提出基于深度和视角方向的alpha调整机制，抑制介质初始化的形成。

Result: 在真实水下和仿真场景实验中，OceanSplat在场景重建和散射介质恢复方面显著优于现有方法，能有效分离散射介质并准确重建物体几何。

Conclusion: OceanSplat通过几何约束和深度感知优化，成功解决了水下3D高斯泼溅的介质纠缠问题，为水下场景重建提供了鲁棒有效的解决方案。
...
TL;DR: 研究了高阶对抗性攻击对目标检测器的影响，发现高阶对抗性补丁具有更强的泛化能力，仅靠对抗训练不足以有效抵御此类攻击。


<details>
  <summary>Details</summary>
Motivation: 当前对抗性攻击与防御之间的博弈呈现持续的循环对抗态势。本研究旨在探究高阶对抗性攻击对目标检测器的影响，特别是通过持续训练攻击模式并结合对抗训练来增强检测器的鲁棒性。

Method: 选择YOLOv10目标检测器作为代表性模型，采用对抗性补丁进行规避攻击。通过连续训练攻击模式和对抗训练来强化目标检测器，研究高阶对抗性攻击的效果。

Result: 结果表明，高阶对抗性补丁不仅影响直接训练的检测器，相比低阶补丁还展现出更强的泛化能力。同时研究发现，单独的对抗训练不足以有效地保护目标检测器抵御此类攻击。

Conclusion: 该研究揭示了对抗性攻击-防御循环中高阶攻击的威胁，强调需要更全面的防御策略来应对不断演化的高阶对抗性攻击。
...
TL;DR: PolyFit是一种基于patch的曲面表示方法，使用局部jet函数拟合曲面patch，可高效学习并推广到各种曲面类型，通过更新少量jet系数实现曲面变形。


<details>
  <summary>Details</summary>
Motivation: 传统的曲面变形方法需要优化每个顶点的自由度，计算量大且效率低。PolyFit旨在提供一种紧凑的表示方法，通过局部拟合和全局优化来实现高效的曲面变形。

Method: 提出PolyFit表示方法，在曲面patch上局部拟合jet函数。通过监督学习从解析函数和真实数据中学习这种表示。学习完成后，只需更新一小部分jet系数即可实现曲面变形。

Result: 在Shape-from-template应用中，PolyFit在保持竞争力的精度的同时，比离线物理求解器快得多，且比最近物理引导的神经模拟器在精度上更好。在服装悬垂应用中，模型在分辨率和服装类型上具有良好泛化能力，推理速度比基线快一个数量级。

Conclusion: PolyFit提供了一种高效、紧凑的曲面表示方法，通过局部拟合和全局优化的结合，在多种应用中实现了快速准确的曲面变形，显著提升了计算效率。
...
TL;DR: 该论文提出了一种专为制药行业设计的视频至高亮片段生成框架，结合音频语言模型和视觉语言模型来高效处理多模态内容。


<details>
  <summary>Details</summary>
Motivation: 制药行业中传统手动标注异构数据模态（文本、图像、视频、音频等）的方法存在不一致性、质量下降和效率低下的问题，长视频和音频数据进一步加剧了这些挑战。

Method: 引入一种领域适应的视频至高亮片段生成框架，整合音频语言模型和视觉语言模型，采用Cut & Merge算法实现平滑过渡和音画对齐，提供基于角色定义和提示注入的个性化机制，以及成本高效的端到端流水线策略。

Result: 在Video MME基准测试和包含16,159个制药视频的专有数据集上评估，实现了3到4倍的速度提升和4倍的成本降低，片段连贯性得分（0.348）和信息性得分（0.721）均优于当前最先进的模型。

Conclusion: 该框架为生命科学领域提供了一种透明、可定制且合规的视频摘要方法，显著提升了处理和利用多模态内容的能力。
...
TL;DR: 该论文提出了一种用于端到端自动驾驶的简单高效的Transformer架构DrivoR，通过相机感知的寄存器token压缩多相机特征，并利用轻量级解码器生成和评估候选轨迹，在多个仿真基准测试中达到或超越了当前先进基准的表现。


<details>
  <summary>Details</summary>
Motivation: 传统自动驾驶方法通常依赖复杂的感知、规划、控制模块，缺乏端到端的优化和效率。本文旨在探索一种纯Transformer架构，通过有效的token压缩技术降低计算开销，同时实现准确、高效的端到端自动驾驶。

Method: 方法基于预训练的视觉Transformer，引入相机感知的寄存器token，将多相机特征压缩为紧凑的场景表示。这些token驱动两个轻量级Transformer解码器：一个生成候选轨迹，另一个通过模拟先验“预言机”（oracle）来为轨迹打分，产生可解释的分数（如安全性、舒适性和效率），并在推理时基于这些子分数进行行为条件驾驶决策。

Result: DrivoR在NAVSIM-v1、NAVSIM-v2和逼真闭环仿真HUGSIM等多个基准测试中，表现优于或匹配了当前先进的基线方法。结果表明，纯Transformer架构结合特定的token压缩技术在保证准确性的同时，有效提升了驾驶效率与适应性。

Conclusion: 研究表明，纯Transformer架构配合目标token压缩能够实现准确、高效且自适应的端到端自动驾驶。该方法证明无需复杂的模块化设计，简单的Transformer模型便能在多相机输入下取得优越的闭环驾驶性能。
...
TL;DR: 提出一种利用激光雷达时序几何一致性自动生成3D伪标签的无监督方法，无需人工标注即可获得语义标签、3D边界框和密集点云，在多个数据集上验证有效性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶应用中，未标注的激光雷达数据数量庞大但缺乏标注，手动标注成本高昂成为感知研究的主要瓶颈。本文旨在通过利用时序几何一致性，将文本和2D视觉基础模型的线索直接提升到3D空间，突破这一瓶颈。

Method: 引入无监督多模态伪标注方法：利用从时序累积的激光雷达地图中学习的强几何先验，结合新颖的迭代更新规则，强制几何-语义联合一致性，并通过不一致性检测移动物体。

Result: 在三个数据集上展示稳健的泛化能力；实验验证方法优于现有需要额外监督的语义分割和物体检测伪标注方法；仅使用少量几何一致的密集激光雷达数据就能在80-150米和150-250米范围内分别提升51.5%和22.0%的深度预测MAE。

Conclusion: 该方法通过无监督方式有效利用激光雷达的时序几何一致性，显著降低3D感知标注成本，并提升深度预测性能，为自动驾驶感知研究提供了实用的解决方案。
...
### [52] [Re-Align: Structured Reasoning-guided Alignment for In-Context Image Generation and Editing](https://arxiv.org/abs/2601.05124)
...
TL;DR: VERSE方法通过分析视觉语言模型的视觉嵌入空间，帮助识别问题区域并生成合成数据来提升视觉丰富文档理解的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在视觉丰富文档理解中存在性能瓶颈，需要一种系统方法来分析和改进模型在视觉特征方面的表现，使开源模型达到或超越私有SaaS解决方案的性能。

Method: VERSE通过可视化模型的潜在表示来评估模型可行性，识别问题区域，并指导生成针对特定视觉特征的合成数据。通过MERIT数据集训练和在MERIT Secret上验证。

Result: VERSE成功揭示了与错误易发集群相关的视觉特征，使用这些特征样本重新训练显著提升了F1性能而不损害泛化能力。优化后的Donut和Idefics2模型性能达到或超越GPT-4和Pixtral等SaaS解决方案。

Conclusion: VERSE为分析和改进视觉语言模型提供了一种有效框架，特别适用于视觉丰富文档理解任务，能够系统性提升模型性能并使开源模型达到商业解决方案水平。
...
TL;DR: 本文提出了VerseCrafter，一种4D感知的视频世界模型，通过新的4D几何控制表示来实现对相机和多个物体动态的统一精确控制。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型难以提供统一且精确的相机和物体动态控制，因为视频本质上是在二维图像平面上操作动态。需要建立连接真实世界环境模拟与精确控制之间的差距。

Method: 使用4D几何控制表示，通过静态背景点云和每个物体3D高斯轨迹编码世界状态；开发自动数据引擎从野生视频提取4D控制，训练视频扩散模型。

Result: 通过渲染这些4D控制作为条件信号，能够生成高保真、视角一致且精确遵循指定动态的视频。

Conclusion: VerseCrafter克服了现有限制，在没有大规模4D标注数据的情况下实现了对视频世界模型中相机和物体动态的统一精确控制。
...
TL;DR: 提出Vision-Language Introspection (VLI)训练免费推理框架，通过自省和双因果调控减少多模态大语言模型中的物体幻觉


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：对比解码方法操作表面化，未能修正内部语义错位；潜在调控方法依赖静态向量，缺乏实例级精确性，无法有效解决视觉-语言不对齐导致的物体幻觉问题

Method: VLI框架包含两个步骤：1）属性自省：通过概率冲突检测诊断幻觉风险并定位因果视觉锚点；2）可解释双因果调控：主动调控推理过程，动态分离视觉证据与背景噪声，通过自适应校准消除盲目置信

Result: 在先进模型上达到最先进性能，在MMHal-Bench上物体幻觉率降低12.67%，在POPE上准确率提升5.8%

Conclusion: VLI框架通过引入认知自省机制，有效缓解多模态大语言模型中的物体幻觉问题，提高模型可靠性和准确率
...
TL;DR: 本文系统研究了多模态大语言模型中思维链推理在视频理解中的作用，提出VideoAuto-R1框架采用按需推理策略，显著提高效率。


<details>
  <summary>Details</summary>
Motivation: 当前视频理解任务中广泛使用思维链推理，但其必要性及相对于直接回答的优势尚未充分探索。作者发现直接回答有时能匹配甚至超越思维链性能，但思维链需要更高的计算成本，因此需要更高效的推理策略。

Method: 提出VideoAuto-R1框架，采用'思考一次，回答两次'的训练范式：首先生成初始答案，随后进行推理，最后输出审核后的答案，两者都通过可验证奖励进行监督。推理时使用初始答案的置信度决定是否进行推理。

Result: 在视频QA和grounding基准测试中，VideoAuto-R1达到最先进的准确率，同时显著提高效率，平均响应长度减少约3.3倍。还观察到感知导向任务中思考模式激活率较低，而推理密集任务中激活率较高。

Conclusion: 显式的语言推理通常有益，但并非总是必要。VideoAuto-R1框架通过按需推理策略实现了准确性和效率的良好平衡，为多模态视频理解提供了更实用的解决方案。
...
TL;DR: 大型视觉语言模型容易出现提示诱导的幻觉，研究表明通过消除特定注意力头可以显著减少这种幻觉


<details>
  <summary>Details</summary>
Motivation: 研究大型视觉语言模型中因偏向文本提示而忽视视觉证据产生的幻觉问题，特别是在对象计数场景中当提示信息高估对象数量时模型的行为

Method: 在受控的对象计数设置中进行实验，通过消融分析识别特定注意力头，研究三种VLMs模型中提示诱导幻觉的机制

Result: 发现少量特定注意力头的消融可以使提示诱导幻觉减少至少40%而不需要额外训练，这些PIH头在不同模型中以不同方式介导提示复制，消融能增加模型对视觉证据的校正

Conclusion: 研究揭示了提示诱导幻觉的内部机制，显示了不同模型在实施这些行为时的特定差异，为理解VLMs的幻觉问题提供了机制性洞见
...
### [63] [RoboVIP: Multi-View Video Generation with Visual Identity Prompting Augments Robot Manipulation](https://arxiv.org/abs/2601.05241)
...
### [64] [GREx: Generalized Referring Expression Segmentation, Comprehension, and Generation](https://arxiv.org/abs/2601.05244)
...
### [65] [Pixel-Perfect Visual Geometry Estimation](https://arxiv.org/abs/2601.05246)
...
### [66] [RL-AWB: Deep Reinforcement Learning for Auto White Balance Correction in Low-Light Night-time Scenes](https://arxiv.org/abs/2601.05249)
...
### [67] [QNeRF: Neural Radiance Fields on a Simulated Gate-Based Quantum Computer](https://arxiv.org/abs/2601.05250)
...
### [68] [Mesh4D: 4D Mesh Reconstruction and Tracking from Monocular Video](https://arxiv.org/abs/2601.05251)
*Zeren Jiang,Chuanxia Zheng,Iro Laina,Diane Larlus,Andrea Vedaldi*

Main category: cs.CV

TL;DR: Mesh4D是一个前馈模型，用于单目4D网格重建，能够从单目视频中一次性重建动态物体的完整3D形状和运动。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单目视频重建动态物体的3D形状和运动时，往往需要复杂的优化过程或对骨骼结构的依赖。作者希望开发一个能够在推理时不需要骨骼信息、且能一次性预测完整动画的高效模型。

Method: 1）构建紧凑的潜在空间，通过自编码器在训练时利用骨骼结构作为先验，学习编码整个动画序列；2）编码器使用时空注意力机制，获得更稳定的物体整体变形表示；3）训练潜在扩散模型，以输入视频和第一帧重建的网格为条件，一次性预测完整动画。

Result: 在重建和新视角合成基准测试中，Mesh4D在恢复准确的3D形状和变形方面优于现有方法，实现了对动态物体形状和运动的完整重建。

Conclusion: Mesh4D通过紧凑潜在空间和潜在扩散模型，实现了从单目视频一次性重建动态物体4D网格的高效方法，无需推理时的骨骼信息，在多个任务上表现出优越性能。
...
### [69] [MedPI: Evaluating AI Systems in Medical Patient-facing Interactions](https://arxiv.org/abs/2601.04195)
...
TL;DR: 本文提出了一个使用大型语言模型(LLM)生成合成电商产品数据的系统方法，通过属性保留修改、可控负例生成和系统属性删除三种策略，显著提升了低资源场景下的产品信息提取性能。


<details>
  <summary>Details</summary>
Motivation: 电商产品信息提取至关重要，但获取高质量标注数据集面临挑战。传统方法在低资源场景下效果有限，需要一种有效的数据增强方案来改善产品信息提取模型的性能。

Method: 引入可控修改框架，采用属性感知提示的先进LLM，实现：1）属性保留修改保持核心信息；2）可控负例生成增强模型鲁棒性；3）系统属性删除模拟真实场景。所有操作都强制遵循店铺约束并保持产品连贯性。

Result: 2000个合成产品的人工评估显示：99.6%被评为自然，96.5%包含有效属性值，超过90%展现一致的属性使用。在MAVE数据集上，合成数据达到60.5%准确率，与真实训练数据（60.8%）相当，远优于零样本基线（13.4%）。混合合成和真实数据的配置进一步提升至68.8%准确率。

Conclusion: 该框架为增强电商数据集提供了实用解决方案，在低资源场景下尤其有价值，能够有效提升产品信息提取模型的性能，且合成数据质量接近于真实数据。
...
TL;DR: TeleTables基准评估大语言模型对电信标准中表格的知识和解释能力，发现小模型在3GPP知识回忆和表格解释上表现不佳，而大模型在表格解释上显示出更强的推理能力，突出了领域专用微调的必要性。


<details>
  <summary>Details</summary>
Motivation: 探讨大语言模型在电信工程任务中的应用时，发现其在处理3GPP标准等电信规范时表现不佳，特别是在处理密集表格信息方面缺乏系统评估，因此需要专门的评测基准。

Method: 通过多级数据生成流水线构建TeleTables基准，从3GPP标准中提取表格，使用多模态和推理导向的大语言模型生成和验证问题，最终形成500个人工验证的问答对及相关表格数据。

Result: 评估显示，小于100亿参数的小模型在3GPP知识回忆和表格解释上都表现不佳，表明其在预训练中接触电信标准有限，且缺乏处理复杂技术材料的归纳偏置；大模型在表格解释上展现出更强的推理能力。

Conclusion: TeleTables基准突出了对电信标准进行可靠解释和推理需要领域专用微调，为改进大语言模型在电信行业的应用提供了重要工具和方向。
...
TL;DR: 本文提出了视觉逼真度对抗文本偏见(V-FAT)基准，用于评估多模态大语言模型是否过度依赖语言先验而非真实视觉基础，并揭示了现有模型在高级别语言主导情境下的视觉崩溃现象。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在标准视觉推理基准上表现优异，但存在过度依赖语言捷径而非真正视觉基础的风险，这种文本偏见现象需要系统性的研究和量化。

Method: 1.将文本偏见解耦为内部语料库偏见和外部指令偏见两个维度；2.构建V-FAT诊断基准，包含4026个VQA实例，覆盖六个语义领域；3.采用三级评估框架，系统性地增加视觉证据与文本信息之间的冲突；4.引入视觉鲁棒性评分(VRS)指标。

Result: 对12个前沿MLLM的评估显示，虽然在现有基准上表现优秀，但在高语言主导情境下出现了显著的视觉崩溃现象。

Conclusion: 多模态大语言模型面临视觉感知与语言先验之间的根本张力，需要在评估和开发中更加关注模型的真实视觉基础能力，避免过度依赖文本偏见。
...
### [75] [FronTalk: Benchmarking Front-End Development as Conversational Code Generation with Multi-Modal Feedback](https://arxiv.org/abs/2601.04203)
...
### [76] [STDD:Spatio-Temporal Dynamics-Driven Token Refinement in Diffusion Language Models](https://arxiv.org/abs/2601.04205)
...
### [77] [Enhancing Admission Inquiry Responses with Fine-Tuned Models and Retrieval-Augmented Generation](https://arxiv.org/abs/2601.04206)
*Aram Virabyan*

Main category: cs.CL

TL;DR: 提出一种结合微调语言模型与检索增强生成（RAG）的AI系统，用于优化大学招生咨询响应时间与信息准确性


<details>
  <summary>Details</summary>
Motivation: 大学招生办公室面临处理大量咨询同时保持回复质量的挑战，现有RAG系统在复杂狭窄的招生领域可能存在局限性，导致上下文不充分的回复

Method: 在专门整理的招生流程数据集上对语言模型进行微调，增强其准确解释RAG检索信息的能力，同时探索响应生成逻辑的优化策略，平衡响应质量与速度

Result: 该混合方法结合了RAG获取最新信息的能力和微调嵌入领域细微理解的优势，提高了招生咨询回复的准确性

Conclusion: 通过定制化微调与RAG技术的结合，可以有效提升大学招生咨询系统的性能，满足特定领域通信需求
...
### [78] [Ideology as a Problem: Lightweight Logit Steering for Annotator-Specific Alignment in Social Media Analysis](https://arxiv.org/abs/2601.04207)
*Wei Xia,Haowen Tang,Luozheng Li*

Main category: cs.CL

TL;DR: 本研究发现LLMs内部的政治意识形态组织与人类意识形态空间存在系统性的不对齐，提出了量化这种不对齐的线性探测方法，并可通过调整输出层概率来对齐用户特定观点，无需重新训练模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型内部的政治意识形态组织与人类意识形态空间之间存在系统性不对齐，这种不对齐会影响到模型输出与用户观点的一致性，因此需要开发低成本、实用且不损害模型原始推理能力的对齐方法。

Method: 设计轻量级线性探针来量化模型内部特征与人类意识形态的不对齐程度，通过计算偏差分数并直接调整最终输出概率来实现对齐，避免重新训练模型。

Result: 方法能够有效测量模型特定的意识形态不对齐，并通过最小化调整输出层来实现与特定用户观点的对齐，同时保持模型的原始推理能力。

Conclusion: 提出的线性探测和输出层调整方法是一种简单、高效、低成本的对齐解决方案，能够在不重新训练模型的情况下实现与用户特定观点的对齐，具有实用性。
...
### [79] [LLMs for Explainable Business Decision-Making: A Reinforcement Learning Fine-Tuning Approach](https://arxiv.org/abs/2601.04208)
*Xiang Cheng,Wen Wang,Anindya Ghose*

Main category: cs.CL

TL;DR: LEXMA是一个基于强化学习的LLM微调框架，为多受众生成高质量、叙述驱动的解释，在抵押贷款审批场景中显著提升预测性能和解释质量。


<details>
  <summary>Details</summary>
Motivation: 当前AI模型决策过程不透明，现有可解释AI技术依赖后验数值特征归因，无法提供连贯的决策叙事；LLM虽能生成自然语言解释，但仍面临三个挑战：解释需要既正确又忠实于预测驱动因素；需服务多受众而不改变决策规则；需标签高效训练而不依赖大规模人工标注解释。

Method: 提出LEXMA框架，结合反思增强的监督微调和两阶段群组相对策略优化（GRPO）：1）微调两个独立参数集分别提高决策正确性和满足不同受众的风格要求；2）使用不依赖人工标注解释的奖励信号进行训练。在抵押贷款审批决策场景中实现该框架。

Result: 实验结果：LEXMA在预测性能上显著优于其他LLM基线；人工评估显示：面向专家的解释更关注风险，面向消费者的解释更清晰、可操作且礼貌。

Conclusion: LEXMA为业务决策提供了成本高效、系统化的LLM微调方法，极大提升了解释质量，为实现可扩展的透明AI系统部署提供了强有力潜力。
...
### [80] [Leveraging Language Models and RAG for Efficient Knowledge Discovery in Clinical Environments](https://arxiv.org/abs/2601.04209)
...
### [81] [Qwerty AI: Explainable Automated Age Rating and Content Safety Assessment for Russian-Language Screenplays](https://arxiv.org/abs/2601.04211)
*Nikita Zmanovskii*

Main category: cs.CL

TL;DR: Qwerty AI 是一个端到端的俄语剧本内容安全评估与分级系统，遵循俄罗斯联邦法律第436-FZ号，能够快速处理长剧本并自动化检测违规内容与分配年龄分级。


<details>
  <summary>Details</summary>
Motivation: 开发自动化系统以满足俄罗斯媒体行业对剧本内容安全合规性评估的需求，解决人工审核效率低和一致性差的问题，特别是在法律规范下确保内容适龄性。

Method: 系统采用微调的Phi-3-mini模型并应用4位量化技术，对剧本进行叙事单元分割（格式依变精度80-95%），并在五大违规内容类别（暴力、性内容、粗俗语言、药物、恐怖元素）上进行检测，最终分配0+到18+的年龄分级并提供可解释的理由。实现中严格限制无外部API调用、80GB VRAM上限，且单剧本处理时间低于5分钟。

Result: 在Wink黑客松（2025年11月）中验证，系统达到80%的分级准确性，分割精度依格式在80%-95%之间，能在2分钟内处理长达700页的剧本，并已部署于Yandex Cloud并利用CUDA加速。

Conclusion: Qwerty AI展示了在法律与技术约束下自动化内容安全评估的可行性，具备实际生产工作流的应用潜力，能够有效应对俄罗斯媒体行业的编辑挑战。
...
### [82] [TrueBrief: Faithful Summarization through Small Language Models](https://arxiv.org/abs/2601.04212)
*Kumud Lakara,Ruibo Shi,Fran Silavong*

Main category: cs.CL

TL;DR: TrueBrief使用偏好优化增强小型语言模型的摘要忠实度，通过受控幻觉注入生成合成偏好数据来减少幻觉问题


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然能生成高质量文本，但在安全关键领域容易产生幻觉，这是一个严重问题。需要特别为小型语言模型开发提升摘要忠实度的框架

Method: 提出端到端框架TrueBrief，基于偏好优化范式，核心是数据生成模块，通过受控幻觉注入技术生成合成偏好数据

Result: 研究深入分析了数据质量和模型大小对基于偏好的优化的影响，强调了这些方法最有效的工作条件

Conclusion: TrueBrief通过生成合成偏好数据和偏好优化，有效提升了小型语言模型在文本摘要任务中的忠实度，为安全关键领域的应用提供了可靠解决方案
...
### [83] [AnimatedLLM: Explaining LLMs with Interactive Visualizations](https://arxiv.org/abs/2601.04213)
...
### [84] [Users Mispredict Their Own Preferences for AI Writing Assistance](https://arxiv.org/abs/2601.04461)
*Vivian Lai,Zana Buçinca,Nil-Jana Akpinar,Mo Houtti,Hyeonsu B. Kang,Kevin Chian,Namjoon Suh,Alex C. Williams*

Main category: cs.CL

TL;DR: 本研究通过对照实验发现，用户对AI写作助手主动介入的偏好主要由组合难度驱动，而非紧迫性；但用户自述偏好与行为存在显著矛盾，导致系统设计若依赖用户自述会降低准确性。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对用户何时需要AI写作助手主动帮助的理解，需要实证探究影响用户偏好的关键因素及其在系统设计中的影响。

Method: 采用析因情境研究设计，邀请50名参与者完成750组配对比较，分析组合复杂度、紧迫性等特征对用户偏好的影响，并对比用户自述偏好与实际行为差异。

Result: 研究发现：1）组合复杂度是主要决策驱动因素（ρ=0.597），紧迫性无预测力（ρ≈0）；2）用户自述偏好与行为存在完全反转的矛盾（自述中紧迫性最重要，但行为中影响最弱）；3）基于用户自述偏好的系统准确率仅57.7%，低于基线，而基于行为模式的系统可达61.3%（p<0.05）。

Conclusion: 依赖用户内省数据进行主动NLG系统设计会产生误导，实际行为数据更可靠，这对前瞻性自然语言生成系统设计有直接意义。
...
### [85] [From Domains to Instances: Dual-Granularity Data Synthesis for LLM Unlearning](https://arxiv.org/abs/2601.04278)
...
### [86] [RIGOURATE: Quantifying Scientific Exaggeration with Evidence-Aligned Claim Evaluation](https://arxiv.org/abs/2601.04350)
...
### [87] [Dialect Matters: Cross-Lingual ASR Transfer for Low-Resource Indic Language Varieties](https://arxiv.org/abs/2601.04373)
...
### [88] [Disco-RAG: Discourse-Aware Retrieval-Augmented Generation](https://arxiv.org/abs/2601.04377)
*Dongqi Liu,Hang Ding,Qiming Feng,Jian Li,Xurong Xie,Zhucun Xue,Chengjie Wang,Jiangning Zhang,Yabiao Wang*

Main category: cs.CL

TL;DR: Disco-RAG：一个通过显式注入篇章结构信号来增强RAG性能的框架，在问答和长文档摘要任务中取得SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 现有RAG策略将检索到的段落视为扁平无结构的信息，限制了模型捕捉结构线索和从分散证据中综合知识的能力。为了解决这些局限，需要引入篇章感知的机制。

Method: Disco-RAG构建内部篇章树捕捉局部层次结构，建立跨段落修辞图建模篇章连贯性，将这些结构联合集成到指导生成的规划蓝图中。

Result: 在问答和长文档摘要基准测试中，Disco-RAG无需微调即达到了最先进的结果，证明了该方法的有效性。

Conclusion: 篇章结构在推进RAG系统发展中扮演着重要角色，显式的篇章信号注入能显著提升生成质量。
...
### [89] [MiJaBench: Revealing Minority Biases in Large Language Models via Hate Speech Jailbreaking](https://arxiv.org/abs/2601.04389)
...
TL;DR: 大语言模型当前的安全评估存在错误的安全普适性假象，作者开发了MiJaBench双语对抗基准，揭示安全对齐存在人口统计层级差异而非通用语义能力，且模型缩放会加剧这些差异。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型安全评估方法通过将'身份仇恨'等指标聚合成标量分数，掩盖了模型对特定群体的系统性脆弱性。作者旨在揭露这种选择性安全现象，挑战安全对齐的普适性假设。

Method: 使用MiJaBench多语言对抗基准数据集，包含44,000个提示覆盖16个少数群体，生成528,000个提示-响应对，并创建MiJaBench-Align评估12个先进LLM的安全对齐表现。

Result: 研究发现安全对齐不是通用语义能力而是人口统计层级：同一模型内对不同群体的防御率变化高达33%；模型缩放会加剧这些差异，表明当前对齐技术并未建立非歧视原则，而是仅强化对特定群体的记忆性拒绝边界。

Conclusion: 当前安全评估方法存在误导性，模型对齐技术实际上加剧了系统性偏见而不是消除歧视，这挑战了安全对齐的普适性假设和当前的安全缩放定律。作者开源了所有数据集和代码以促进精细化人口统计对齐研究。
...
### [90] [Accommodation and Epistemic Vigilance: A Pragmatic Account of Why LLMs Fail to Challenge Harmful Beliefs](https://arxiv.org/abs/2601.04435)
...
### [91] [Learning to Simulate Human Dialogue](https://arxiv.org/abs/2601.04436)
*Kanishk Gandhi,Agam Bhatia,Noah D. Goodman*

Main category: cs.CL

TL;DR: 研究表明，优化基于评判者的奖励虽然能提高判断分数，但会降低模型对真人对话的预测能力；而直接最大化对数概率能更好地预测人类实际说话方式，其中将思维链作为隐变量优化，能得到最佳评估结果。


<details>
  <summary>Details</summary>
Motivation: 通过预测人类下一轮对话来建模人类的思考方式，比较不同学习方法的有效性，特别是思考与否对模型预测能力的影响。

Method: 比较两种学习维度：一是模型是否允许思考后再回应；二是通过LLM评判者评分（语义相似性和信息完整性）还是直接最大化真实人类对话的对数概率。推导思维链作为隐变量的对数概率下界并优化该目标。

Result: 优化评判奖励能提高评判分数，但会降低对人类真实响应的预测可能性；直接最大化对数概率的方法在概率和人类偏好评估中表现更好；将思维链作为隐变量优化在所有评估中获得最佳结果。

Conclusion: 思考主要在与真实人类对话分布匹配的目标训练时有帮助，将此方法扩展到更广泛的对话数据可能产生对人类行为有更细致理解的模型。
...
### [92] [Merging Triggers, Breaking Backdoors: Defensive Poisoning for Instruction-Tuned Language Models](https://arxiv.org/abs/2601.04448)
*San Kim,Gary Geunbae Lee*

Main category: cs.CL

TL;DR: 提出MB-Defense防御框架，通过两阶段训练流程免疫指令调优LLMs抵抗后门攻击，包含防御性下毒和权重恢复阶段，有效降低攻击成功率且保持指令遵循能力。


<details>
  <summary>Details</summary>
Motivation: 指令调优的大语言模型依赖大规模数据集，容易受到后门攻击，目前对其防御研究不足，需要开发可靠的防御方法来提高模型的鲁棒性。

Method: MB-Defense包含两个阶段：1)防御性下毒阶段：将攻击者和防御性触发器合并为统一的后门表示；2)权重恢复阶段：通过额外训练破坏该表示并恢复干净行为。

Result: 在多个大语言模型上进行广泛实验，MB-Defense显著降低攻击成功率，同时保持模型的指令遵循能力，对未见过的后门攻击也具有可泛化性。

Conclusion: MB-Defense为增强指令调优大语言模型的鲁棒性提供了高效、可泛化的防御策略，能够有效抵抗多样的后门威胁。
...
### [93] [Beyond Static Summarization: Proactive Memory Extraction for LLM Agents](https://arxiv.org/abs/2601.04463)
*Chengyuan Yang,Zequn Sun,Wei Wei,Wei Hu*

Main category: cs.CL

TL;DR: 该论文提出了ProMem方法，通过自我提问的循环反馈机制解决传统LLM记忆提取中因提前总结和单向提取导致的信息丢失问题。


<details>
  <summary>Details</summary>
Motivation: 现有的摘要式记忆提取方法存在两个主要限制：1) 总结过程是'超前'的单向前馈过程，会因不了解未来任务而遗漏重要细节；2) 提取通常是'一次性'的，缺乏验证事实的反馈循环，导致信息丢失累积。

Method: 提出主动记忆提取方法ProMem，将提取视为迭代认知过程。通过引入循环反馈机制，智能体使用自我提问主动探查对话历史，从而恢复缺失信息和纠正错误。

Result: ProMem显著提高了提取记忆的完整性和QA准确性，同时在提取质量和token成本之间实现了更优的平衡。

Conclusion: 将记忆提取从静态摘要转变为动态认知过程能有效解决现有方法的局限性，循环反馈机制是关键改进方向。
...
### [94] [Concept Tokens: Learning Behavioral Embeddings Through Concept Definitions](https://arxiv.org/abs/2601.04465)
...
TL;DR: 为芬兰语、匈牙利语和爱沙尼亚语等形态丰富的乌拉尔语族语言提出了一个无需语料库的形态词典生成工具包SampoNLP，并以此词典为基础系统地评估了BPE分词器，提出了统一的综合性能得分（IPS）来衡量词素覆盖率和过度切分之间的权衡，并首次给出了这些语言的最佳词表大小的经验建议。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的子词分词质量至关重要，但对于形态丰富的乌拉尔语族语言来说，由于缺乏干净的词素词典，评估分词器一直存在困难。

Method: 提出了SampoNLP工具包，该工具包使用基于最小描述长度（MDL）启发的自参考原子性评分方法，无需语料库即可生成高纯度的形态词典。然后使用该词典对三个乌拉尔语族语言在不同词表大小（8k-256k）下的BPE分词器进行了系统评估，并提出综合性能得分（IPS）来平衡词素覆盖率和过度切分。通过分析IPS曲线，识别了收益递减的“拐点”，从而得出最优词表大小。

Result: 为芬兰语、匈牙利语和爱沙尼亚语生成了高质量的形态词典，并利用IPS分析首次为这些语言提供了基于实证的最优BPE词表大小的推荐。量化研究表明标准BPE对于高度黏着性语言存在局限性。

Conclusion: SampoNLP工具包和相关资源已公开；这项研究不仅为乌拉尔语族语言的分词提供了实用指导，还定量地证明了标准BPE在处理高度黏着性语言时的不足。基于IPS的分析方法为分词器评估提供了一种统一的衡量标准，并为未来其他语言的研究奠定了基础。
...
### [97] [LinguaGame: A Linguistically Grounded Game-Theoretic Paradigm for Multi-Agent Dialogue Generation](https://arxiv.org/abs/2601.04516)
...
### [98] [GRACE: Reinforcement Learning for Grounded Response and Abstention under Contextual Evidence](https://arxiv.org/abs/2601.04525)
...
### [99] [BanglaLorica: Design and Evaluation of a Robust Watermarking Algorithm for Large Language Models in Bangla Text Generation](https://arxiv.org/abs/2601.04534)
...
### [100] [Identifying Good and Bad Neurons for Task-Level Controllable LLMs](https://arxiv.org/abs/2601.04548)
*Wenjie Li,Guansong Pang,Hezhe Qiao,Debin Gao,David Lo*

Main category: cs.CL

TL;DR: 本文提出了NeuronLLM框架，首次将生物学的功能拮抗原理应用于大型语言模型神经元识别，通过对比学习建模促进和抑制任务完成的神经元，解决现有方法局限于单一能力、忽视抑制作用及随机行为误导的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多项选择题评测中表现出色，但其大规模神经元的复杂机制不透明，现有方法仅关注与特定能力相关的支持性神经元，无法处理需要多种能力协同的任务场景，且忽视抑制性神经元角色以及模型随机行为导致的错误归因。

Method: NeuronLLM框架基于功能拮抗原理，将神经元分为促进任务完成的'好神经元'和抑制任务完成的'坏神经元'，通过对比学习对两类神经元进行整体建模，并利用增强的问题集减轻LLM的随机行为干扰。

Result: 在不同规模和家族的LLM上的综合实验表明，NeuronLLM在四个NLP任务上优于现有方法，为理解LLM的功能组织提供了新的见解。

Conclusion: NeuronLLM通过引入功能拮抗原理和对比学习，实现了任务级别的LLM神经元识别，为理解和操控大型语言模型提供了更全面有效的框架。
...
### [101] [FeedEval: Pedagogically Aligned Evaluation of LLM-Generated Essay Feedback](https://arxiv.org/abs/2601.04574)
...
### [102] [Aligning Text, Code, and Vision: A Multi-Objective Reinforcement Learning Framework for Text-to-Visualization](https://arxiv.org/abs/2601.04582)
*Mizanur Rahman,Mohammed Saidul Islam,Md Tahmid Rahman Laskar,Shafiq Joty,Enamul Hoque*

Main category: cs.CL

TL;DR: 提出 RL-Text2Vis 框架，使用强化学习提升文本到可视化的生成质量，相比 GPT-4o 在图表质量上提升 22%，代码执行成功率从 78% 提升至 97%。


<details>
  <summary>Details</summary>
Motivation: 现有文本到可视化系统常生成语义不匹配的图表，开源模型更是难以生成可执行代码。监督微调虽能提升代码可执行性，但无法利用执行后的反馈来优化可视化质量。

Method: 基于 Group Relative Policy Optimization (GRPO) 的强化学习框架，设计多目标奖励函数，综合考虑文本准确性、代码有效性和执行后的可视化质量。

Result: 在 Text2Vis 基准上，图表质量提升 22%，代码执行成功率从 78% 提高至 97%。在 VIS-Eval、NVBench 等跨领域数据集也展现优秀泛化能力。

Conclusion: GRPO 是提升可视化生成中结构化、多模态推理的有效策略，强化学习框架 RL-Text2Vis 显著优于现有零样本和监督基线方法。
...
### [103] [THaLLE-ThaiLLM: Domain-Specialized Small LLMs for Finance and Thai -- Technical Report](https://arxiv.org/abs/2601.04597)
...
TL;DR: 该研究探讨了通过模型合并技术，将不同的专门化模型（如基础模型和泰国语增强模型）整合为单一高性能、多能力的大语言模型，以资源高效的方式满足金融等行业需求。


<details>
  <summary>Details</summary>
Motivation: 由于隐私、安全和监管要求，金融机构倾向于本地部署大语言模型，但传统方法需要在多个专门化模型和昂贵的大型多功能模型之间权衡成本与性能；泰国语LLM项目旨在提升开放大语言模型的泰语能力，但如何经济高效地实现多功能模型成为挑战。

Method: 采用模型合并（model merging）技术，进行了两项实验：1) 合并Qwen-8B与ThaiLLM-8B以提升泰语通用能力；2) 进一步合并Qwen-8B、ThaiLLM-8B和THaLLE-CFA-8B，以同时增强通用和金融领域能力。

Result: 实验一显示，合并后模型在泰国M3和M6 O-NET考试中表现优于基础模型Qwen-8B；实验二进一步提升了在O-NET、Flare-CFA金融评估和泰国信息抽取基准上的综合性能。

Conclusion: 模型合并是资源高效地创建高性能、多能力大语言模型的可行方案，能够平衡成本与性能需求，为行业应用提供了实用路径。
...
### [104] [On the Limitations of Rank-One Model Editing in Answering Multi-hop Questions](https://arxiv.org/abs/2601.04600)
*Zhiyuan He,Binghan Chen,Tianxiang Xiong,Ziyang Sun,Mozhao Zhu,Xi Chen*

Main category: cs.CL

TL;DR: 研究发现ROME等多跳知识编辑方法存在'跳转过晚'、泛化衰减和过拟合问题。提出的冗余编辑策略通过在多层进行编辑，显著提升多跳推理准确率，但牺牲了部分特异性和自然性。


<details>
  <summary>Details</summary>
Motivation: 知识编辑技术如ROME在更新单跳事实时表现出色，但在处理需要知识链的多跳推理任务时面临显著挑战。现有方法在深层编辑时会出现'跳转过晚'等问题，导致多跳推理性能下降。

Method: 提出了Redundant Editing冗余编辑策略来缓解'跳转过晚'和泛化能力衰减问题。该方法通过在多层进行编辑来增强多跳推理能力。

Result: 实验显示该方法可以将2跳问题的准确率提高至少15.5个百分点，相比之前的单次编辑策略提升了96%，但在特异性和语言自然性方面有所折衷。

Conclusion: 冗余编辑策略能有效提升知识编辑方法在多跳推理任务上的性能，特别是在深层编辑时，为解决知识编辑中的多跳推理问题提供了实用方案。
...
### [105] [When More Words Say Less: Decoupling Length and Specificity in Image Description Evaluation](https://arxiv.org/abs/2601.04609)
...
### [106] [Character-R1: Enhancing Role-Aware Reasoning in Role-Playing Agents via RLVR](https://arxiv.org/abs/2601.04611)
...
### [107] [From National Curricula to Cultural Awareness: Constructing Open-Ended Culture-Specific Question Answering Dataset](https://arxiv.org/abs/2601.04632)
*Haneul Yoo,Won Ik Cho,Geunhye Kim,Jiyoon Han*

Main category: cs.CL

TL;DR: 提出了一个自动化多智能体LLM框架，将国家教科书课程转化为开放式文化特定问答对，用于增强AI文化对齐。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在文化和语言间的表现不平衡，常反映其英语中心训练数据的价值观，因此需要实用的文化对齐方法。

Method: 提出CuCu框架，基于国家社科课程自动生成开放式文化特定问答对；将其应用于韩国社科课程构建了34.1k问答对的数据集KCaQA。

Result: 定性和定量分析显示KCaQA覆盖了文化特定主题，并能基于当地社会文化背景生成响应。

Conclusion: 通过国家课程进行文化监督可扩展实际文化对齐；CuCu展示了自动化构建文化基础数据集的潜力。
...
### [108] [MAGA-Bench: Machine-Augment-Generated Text via Alignment Detection Benchmark](https://arxiv.org/abs/2601.04633)
...
### [109] [SpeechMedAssist: Efficiently and Effectively Adapting Speech Language Models for Medical Consultation](https://arxiv.org/abs/2601.04638)
...
### [110] [CRANE: Causal Relevance Analysis of Language-Specific Neurons in Multilingual Large Language Models](https://arxiv.org/abs/2601.04664)
*Yifan Le,Yunliang Li*

Main category: cs.CL

TL;DR: 提出CRANE框架，基于功能必要性而非激活强度来定义语言特异性，通过定向神经元干预更精确地识别多语言大模型中的语言特定神经元。


<details>
  <summary>Details</summary>
Motivation: 目前多语言大语言模型在多语言任务上表现良好，但神经元层面如何组织语言能力尚不清楚。先前基于激活启发式的方法混淆了语言偏好与功能重要性，需要更准确的分析框架。

Method: 提出CRANE框架，通过定向神经元干预识别语言特异神经元，基于神经元对语言条件预测的贡献来衡量其特异性，而非激活幅度。

Result: 神经元干预揭示了非对称模式：掩盖目标语言相关神经元会选择性降低该语言性能，同时大幅保留其他语言性能，表明存在语言选择性但非排他的神经元专化。在英、中、越语基准测试中，CRANE比基于激活的方法更精确地分离语言特定组件。

Conclusion: CRANE框架从功能必要性角度重新定义语言特异性，通过神经元干预提供更精确的多语言能力分析工具，揭示了神经元层面语言能力的组织模式。
...
### [111] [ToolGate: Contract-Grounded and Verified Tool Execution for LLMs](https://arxiv.org/abs/2601.04688)
...
### [112] [Thunder-KoNUBench: A Corpus-Aligned Benchmark for Korean Negation Understanding](https://arxiv.org/abs/2601.04693)
*Sungmok Jung,Yeonkyoung So,Joonhak Lee,Sangho Kim,Yelim Ahn,Jaejin Lee*

Main category: cs.CL

TL;DR: 该研究针对韩语中否定现象对LLM的挑战，构建了反映韩语否定现象分布的语料库基准Thunder-KoNUBench，评估了47个模型并分析了模型规模和指令调优的影响，证明在该基准上的微调能提升韩语否定理解和更广泛的上下文理解能力。


<details>
  <summary>Details</summary>
Motivation: 尽管已知否定对大型语言模型具有挑战性，但评估否定理解的基准在韩语中尤为稀缺。因此需要构建专门针对韩语否定现象的评估基准来填补这一空白。

Method: 首先进行韩语否定现象的语料库分析，揭示LLMs在否定条件下的性能下降。然后构建句子级基准Thunder-KoNUBench，该基准反映了韩语否定现象的实证分布。最后评估了47个LLMs，分析模型规模和指令调优的影响，并在该基准上进行微调实验。

Result: 研究表明LLMs在韩语否定理解方面性能下降显著。通过Thunder-KoNUBench评估发现，模型规模和指令调优对否定理解有重要影响。更重要的是，在该基准上的微调不仅能提升否定理解能力，还能改善更广泛的韩语上下文理解能力。

Conclusion: 该研究填补了韩语否定理解评估基准的空白，证明了专门构建的语言特定基准对提升LLMs语言理解能力的重要性。Thunder-KoNUBench为评估和改善LLMs的韩语否定理解提供了有效工具，微调实验表明这种方法能够提升模型在目标语言任务上的整体表现。
...
### [113] [Prior-Informed Zeroth-Order Optimization with Adaptive Direction Alignment for Memory-Efficient LLM Fine-Tuning](https://arxiv.org/abs/2601.04710)
...
### [114] [DSC2025 -- ViHallu Challenge: Detecting Hallucination in Vietnamese LLMs](https://arxiv.org/abs/2601.04711)
...
### [115] [Qwen3-VL-Embedding and Qwen3-VL-Reranker: A Unified Framework for State-of-the-Art Multimodal Retrieval and Ranking](https://arxiv.org/abs/2601.04720)
...
### [116] [Automatic Classifiers Underdetect Emotions Expressed by Men](https://arxiv.org/abs/2601.04730)
...
### [117] [AM$^3$Safety: Towards Data Efficient Alignment of Multi-modal Multi-turn Safety for MLLMs](https://arxiv.org/abs/2601.04736)
*Han Zhu,Jiale Chen,Chengkun Cai,Shengjie Sun,Haoran Li,Yujin Zhou,Chi-Min Chan,Pengcheng Wen,Lei Li,Sirui Han,Yike Guo*

Main category: cs.CL

TL;DR: 该研究提出了InterSafe-V多模态对话安全数据集和AM³Safety框架，用于增强多模态大语言模型在多轮对话中的安全性，显著降低攻击成功率并提升无害性和帮助性


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在交互应用中的部署日益增加，但在多轮多模态场景下存在严重的安全漏洞：有害意图可能在多轮对话中逐步重建，安全协议随着对话进行逐渐失效。现有的RLHF对齐方法主要针对单轮视觉问答任务设计，且需要昂贵的人工偏好标注，在对话场景中效果和可扩展性有限

Method: 提出了InterSafe-V开源多模态对话数据集（包含11,270个对话和500个专门设计的拒绝VQA样本）；AM³Safety框架结合冷启动拒绝阶段和基于组相对策略优化的微调，使用回合感知的双目标奖励函数在完整对话上进行优化

Result: 在Qwen2.5-VL-7B-Instruct和LLaVA-NeXT-7B模型上的实验表明：攻击成功率降低超过10%，在无害性维度提升至少8%，在帮助性维度提升超过13%，同时保持模型的一般能力

Conclusion: 该研究为解决多模态大语言模型在多轮对话中的安全脆弱性问题提供了有效的数据集和框架，能够在提升安全性的同时保持模型的实用能力，为多模态对话系统的安全对齐提供了新思路
...
### [118] [RiskAtlas: Exposing Domain-Specific Risks in LLMs through Knowledge-Graph-Guided Harmful Prompt Generation](https://arxiv.org/abs/2601.04740)
...
### [119] [PILOT-Bench: A Benchmark for Legal Reasoning in the Patent Domain with IRAC-Aligned Classification Tasks](https://arxiv.org/abs/2601.04758)
*Yehoon Jang,Chaewon Lee,Hyun-seok Min,Sungchul Choi*

Main category: cs.CL

TL;DR: 本文提出了PILOT-Bench，这是第一个基于PTAB的基准测试数据集，用于评估LLM在专利法律领域的结构化推理能力，并揭示了闭源模型与开源模型之间的性能差距。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在专利和法律领域应用日益广泛，但现有研究主要限于轻量级任务，缺乏系统评估其在专利领域结构化法律推理能力的方法。

Method: 构建了PILOT-Bench基准数据集，将PTAB决定与USPTO专利数据进行案例级对齐，并设计了三个基于IRAC框架的分类任务（问题类型、委员会权威依据、子决定），评估了一系列闭源和开源LLM。

Result: 闭源模型在问题类型任务上的Micro-F1分数持续超过0.75，而最强的开源模型（Qwen-8B）性能约为0.56，显示出明显的推理能力差距。

Conclusion: PILOT-Bench为系统评估专利领域法律推理能力奠定了基础，并指出了通过数据集设计和模型对齐来改进LLM的未来方向。
...
### [120] [Differential syntactic and semantic encoding in LLMs](https://arxiv.org/abs/2601.04765)
...
### [121] [Revisiting Judge Decoding from First Principles via Training-Free Distributional Divergence](https://arxiv.org/abs/2601.04766)
...
### [122] [LANGSAE EDITING: Improving Multilingual Information Retrieval via Post-hoc Language Identity Removal](https://arxiv.org/abs/2601.04768)
...
### [123] [NC2C: Automated Convexification of Generic Non-Convex Optimization Problems](https://arxiv.org/abs/2601.04789)
...
### [124] [Belief in Authority: Impact of Authority in Multi-Agent Evaluation Framework](https://arxiv.org/abs/2601.04790)
*Junhyuk Choi,Jeongyoun Kwon,Heeju Kim,Haeun Cho,Hayeong Jung,Sehee Min,Bugeun Kim*

Main category: cs.CL

TL;DR: 系统分析了多智能体系统中基于角色的权威偏见，发现专家型和参照型权力比法定型权力影响更强，权威偏见主要源于权威角色坚持立场而非普通智能体被动服从，且需明确表态才能产生偏见影响。


<details>
  <summary>Details</summary>
Motivation: 现有使用大语言模型的多智能体系统常分配权威角色提升性能，但权威偏见对智能体互动的影响尚缺乏系统研究。本文旨在填补这一空白。

Method: 利用ChatEval平台，基于French和Raven的权力理论，将权威角色分为法定型、参照型和专家型三类，分析它们在12轮对话中的影响。使用GPT-4o和DeepSeek R1模型进行实验。

Result: 实验显示：1.专家型和参照型权力角色比法定型权力角色影响力更强；2.权威偏见主要源于权威角色坚持立场而非普通智能体被动服；3.权威影响力需要明确的立场声明，中性回应无法产生偏见。

Conclusion: 这些发现为设计具有不对称互动模式的多智能体框架提供了重要见解，应关注权威角色的类型差异及其表达立场的方式对系统互动的影响。
...
### [125] [When AI Settles Down: Late-Stage Stability as a Signature of AI-Generated Text Detection](https://arxiv.org/abs/2601.04833)
*Ke Sun,Guangsheng Bao,Han Cui,Yue Zhang*

Main category: cs.CL

TL;DR: 提出了基于晚期阶段统计的零样本AI生成文本检测新方法，发现了AI文本生成中的时间动态规律：后期波动衰减现象，并通过简单特征实现了最先进的检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统零样本检测方法聚合整个序列的标记级统计信息，忽略了自回归生成过程中固有的时间动态特性。研究发现AI生成文本和人类写作在波动模式上存在显著差异。

Method: 1）分析12万多个文本样本，发现晚期波动衰减现象：AI生成的文本在生成后期呈现迅速稳定化的对数概率波动，而人类写作始终保持较高的变异性；2）基于此发现提出两个简单特征：导数离散度和局部波动度，仅从晚期统计信息计算；3）无需扰动采样或额外的模型访问。

Result: 1）在序列后半段，AI生成文本比人类文本波动低24-32%；2）新方法在EvoBench和MAGEDe评测中达到了最先进的性能；3）与现有全局方法表现出很强的互补性。

Conclusion: 通过关注文本生成的时间动态特性，特别是晚期波动衰减现象，仅使用简单的晚期统计特征就能实现高效的AI文本检测，且该方法与现有方法具有良好的互补性。
...
### [126] [RAAR: Retrieval Augmented Agentic Reasoning for Cross-Domain Misinformation Detection](https://arxiv.org/abs/2601.04853)
...
### [127] [Token Maturation: Autoregressive Language Generation via Continuous Token Dynamics](https://arxiv.org/abs/2601.04854)
*Oshri Naparstek*

Main category: cs.CL

TL;DR: 提出了一个连续自回归语言生成模型，通过多维连续向量逐步“成熟”演化而非直接离散化采样，能够实现稳定的确定性生成


<details>
  <summary>Details</summary>
Motivation: 传统自回归模型早期硬性离散化（采样）导致不稳定性、重复性和敏感性问题，需要在连续空间中更好地管理和解决不确定性

Method: 使用连续向量表示token，通过确定性动态过程逐步更新并“成熟”，仅当表示充分收敛时才硬解码为离散token，保持不确定性在连续空间中解决

Result: 仅依赖成熟过程而不需token级采样、扩散式去噪或辅助稳定机制，就能产生连贯多样的文本（通过argmax确定性解码）

Conclusion: 这是首个通过演化连续token表示至收敛、再离散化的自回归语言模型，实现了无需token级采样的稳定生成
...
### [128] [MisSpans: Fine-Grained False Span Identification in Cross-Domain Fake News](https://arxiv.org/abs/2601.04857)
...
### [129] [A Navigational Approach for Comprehensive RAG via Traversal over Proposition Graphs](https://arxiv.org/abs/2601.04859)
...
### [130] [EvolSQL: Structure-Aware Evolution for Scalable Text-to-SQL Data Synthesis](https://arxiv.org/abs/2601.04875)
*Xuanguang Pan,Chongyang Tao,Jiayuan Bai,Jianling Gao,Zhengwei Tao,Xiansheng Zhou,Gavin Cheung,Shuai Ma*

Main category: cs.CL

TL;DR: TL;DR: EvolSQL框架通过结构感知的数据合成，从种子数据演化SQL查询，解决了Text-to-SQL数据稀缺问题，显著提升了模型性能


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL方法依赖有限标注数据或简单合成数据，导致SQL结构多样性有限复杂度不足。为解决高质量、多样化且结构复杂数据稀缺的问题，提出EvolSQL框架。

Method: 1. 探索性Query-SQL扩展：扩展查询多样性，提升模式覆盖；2. 自适应定向演化：使用六个基于SQL抽象语法树的原子操作符，在关系、谓词、聚合和嵌套等维度上逐步增加查询复杂度；3. 基于执行的SQL精炼模块和模式感知去重。

Result: 实验证明，使用EvolSQL合成数据微调的7B模型，仅使用SynSQL数据的1/18，就能在性能上超越基于更大规模SynSQL数据集训练的模型。

Conclusion: EvolSQL框架能够高效生成高质量、结构多样的训练数据，显著提升Text-to-SQL模型性能，为解决数据稀缺问题提供了有效方案。
...
### [131] [Mind2Report: A Cognitive Deep Research Agent for Expert-Level Commercial Report Synthesis](https://arxiv.org/abs/2601.04879)
...
### [132] [Faithful Summarisation under Disagreement via Belief-Level Aggregation](https://arxiv.org/abs/2601.04889)
*Favour Yahdii Aghaebe,Tanefa Apekey,Elizabeth Williams,Nafise Sadat Moosavi*

Main category: cs.CL

TL;DR: 本文提出了一种分歧感知的摘要生成方法，将信念层聚合与语言生成分离，以解决LLM在处理多文档中的冲突观点时过度平滑分歧的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的意见和多文档摘要方法（特别是LLM系统）在处理真正冲突的观点时，往往会隐式地平滑分歧，过度代表多数观点，这限制了在意见密集型场景中生成摘要的忠实性。

Method: 采用分歧感知的合成流程：首先将文档表示为结构化信念集，使用基于距离的信念合并算子进行显式冲突建模的聚合；然后仅使用大语言模型将聚合后的信念转化为自然语言摘要。

Result: 评估结果显示，足够大的模型在生成时进行显式聚合可以匹配信念层聚合的性能，但这种行为在不同架构或能力模型间不稳定。相比之下，信念层聚合结合简单提示能在不同模型中持续提供强大的分歧感知性能，同时保持流畅和基于事实的摘要。

Conclusion: 将信念层聚合与语言生成分离的方法是稳健的，能够确保摘要忠实反映文档中的冲突观点，而不仅仅依赖于单一LLM的生成能力。
...
### [133] [Can AI-Generated Persuasion Be Detected? Persuaficial Benchmark and AI vs. Human Linguistic Differences](https://arxiv.org/abs/2601.04925)
*Arkadiusz Modzelewski,Paweł Golik,Anna Kołos,Giovanni Da San Martino*

Main category: cs.CL

TL;DR: 本文研究了LLM生成的说服性文本是否比人工写作更难自动检测，发现明显说服性内容较易检测，但隐蔽性说服会降低检测性能，创建了多语言基准并进行了语言分析


<details>
  <summary>Details</summary>
Motivation: 大型语言模型能够生成高度说服性的文本，这引发了对其被滥用于宣传、操纵等有害目的的担忧，需要研究LLM生成的说服性内容是否比人工写作更难检测

Method: 对LLM生成说服性内容的可控生成方法进行分类，构建Persuaficial基准（涵盖英、德、波兰、意、法、俄六种语言），通过该基准对比人工写作和LLM生成说服性文本的检测效果，并进行全面的语言学分析

Result: 研究发现：虽然明显的LLM生成说服性文本比人工写作更容易检测，但隐蔽的LLM生成说服会持续降低自动检测性能

Conclusion: 研究提供了首个全面对比人工和LLM生成说服性文本的语言学分析，为开发更可解释和更强的检测工具提供了理论基础
...
### [134] [GenProve: Learning to Generate Text with Fine-Grained Provenance](https://arxiv.org/abs/2601.04932)
...
### [135] [A Unified Spoken Language Model with Injected Emotional-Attribution Thinking for Human-like Interaction](https://arxiv.org/abs/2601.04960)
*Qing Wang,Zehan Li,Yaodong Song,Hongjie Chen,Jian Kang,Jie Lian,Jie Li,Yongxiang Li,Xuelong Li*

Main category: cs.CL

TL;DR: 提出融入情感归因思维的统一口语语言模型，通过两阶段训练策略实现了情感感知推理，在情感对话评测中获得最佳表现。


<details>
  <summary>Details</summary>
Motivation: 现有口语对话系统缺乏深度情感理解能力，需要在推理过程中内在化用户情感状态及其成因，而非仅作为外部监督信号。

Method: 采用IEAT数据构造策略，将用户情感状态及成因注入模型推理过程；两阶段训练：第一阶段通过自蒸馏实现语音-文本对齐和情感属性建模，第二阶段进行端到端跨模态联合优化确保文本与语音情感表达一致性。

Result: 在HumDial情感智能评测中，在情感轨迹建模、情感推理和共情回复生成任务上均获得LLM评估和人工评估的排名第一。

Conclusion: IEAT策略和两阶段训练方法有效提升了口语语言模型的情感智能，实现了情感感知的内在化推理和多模态情感表达的一致性。
...
### [136] [Text as a Universal Interface for Transferable Personalization](https://arxiv.org/abs/2601.04963)
...
### [137] [Learning from Mistakes: Negative Reasoning Samples Enhance Out-of-Domain Generalization](https://arxiv.org/abs/2601.04992)
*Xueyun Tian,Minghua Ma,Bingbing Xu,Nuoyan Lyu,Wei Li,Heng Dong,Zheng Chu,Yuanzhuo Wang,Huawei Shen*

Main category: cs.CL

TL;DR: 本研究提出GLOW方法，通过利用含错误答案轨迹改进监督微调，提升大语言模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统监督微调只使用正确答案轨迹，忽视了错误轨迹中有效的中间推理信息。这些被丢弃的监督信号可能导致过拟合，限制模型的域外泛化能力。

Method: 引入包含负向轨迹的监督微调，并提出基于增益的损失权重调整方案GLOW，根据样本跨epoch训练进展自适应重标损失权重。

Result: 相比仅使用正向轨迹的训练，加入负向轨迹带来显著域外泛化提升（Qwen2.5-7B上5.51%增益），推理时策略熵提升35.67%，MMLU从72.82%提升至76.47%。

Conclusion: 充分利用负向轨迹的监督微调能有效提升模型泛化能力，GLOW方法通过自适应权重调整进一步优化了这一过程，为语言模型推理能力训练提供了新思路。
...
### [138] [Can Large Language Models Resolve Semantic Discrepancy in Self-Destructive Subcultures? Evidence from Jirai Kei](https://arxiv.org/abs/2601.05004)
...
### [139] [Hán Dān Xué Bù (Mimicry) or Qīng Chū Yú Lán (Mastery)? A Cognitive Perspective on Reasoning Distillation in Large Language Models](https://arxiv.org/abs/2601.05019)
...
### [140] [ArcAligner: Adaptive Recursive Aligner for Compressed Context Embeddings in RAG](https://arxiv.org/abs/2601.05038)
...
### [141] [Compositional Steering of Large Language Models with Steering Tokens](https://arxiv.org/abs/2601.05062)
...
### [142] [SemPA: Improving Sentence Embeddings of Large Language Models through Semantic Preference Alignment](https://arxiv.org/abs/2601.05075)
*Ziyang Chen,Zhenxuan Huang,Yile Wang,Weiqin Wang,Lu Yin,Hui Huang*

Main category: cs.CL

TL;DR: 本研究提出SemPA方法，通过语义偏好对齐提升句子表征能力，同时保持LLMs的生成能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于生成式大语言模型的嵌入方法存在局限：固定提示模板缺乏模型优化，性能有限；修改架构会破坏模型内部计算机制，损害生成能力。

Method: 采用句子级的直接偏好优化在释义生成任务上高效优化LLMs，让模型学习区分语义等效句子而不牺牲固有生成能力。

Result: 在语义文本相似性任务和多项LLMs基准测试中，SemPA实现了更好的语义表征，同时保持了LLMs的固有生成能力。

Conclusion: SemPA通过语义偏好对齐有效地提升了句子表征质量，且不损害大语言模型的生成能力，为提升语义理解提供了新思路。
...
### [143] [Code-Mix Sentiment Analysis on Hinglish Tweets](https://arxiv.org/abs/2601.05091)
*Aashi Garg,Aneshya Das,Arshi Arya,Anushka Goyal,Aditi*

Main category: cs.CL

TL;DR: 针对印度社交媒体上广泛使用的Hinglish（印地语-英语混合语）品牌监测挑战，提出基于mBERT的高性能情感分类框架


<details>
  <summary>Details</summary>
Motivation: 传统单语NLP模型难以处理Hinglish这种代码混合语言的句法和语义复杂性，导致情感分析不准确和市场洞察误导，需要专门解决方案

Method: 通过微调多语言BERT（mBERT）模型，利用其多语言能力理解印度社交媒体语言多样性；关键方法是使用子词分词有效处理罗马化Hinglish中常见的拼写变体、俚语和未登录词

Result: 开发出可用于生产的品牌情感追踪AI解决方案，为低资源代码混合环境的多语言NLP建立了强基准

Conclusion: 该研究成功解决了Hinglish情感分析的技术挑战，为印度市场品牌监测提供了高效工具，推动了多语言NLP在代码混合环境中的应用
...
### [144] [How Human is AI? Examining the Impact of Emotional Prompts on Artificial and Human and Responsiveness](https://arxiv.org/abs/2601.05104)
...
### [145] [Agent-as-a-Judge](https://arxiv.org/abs/2601.05111)
*Runyang You,Hongru Cai,Caiqi Zhang,Qiancheng Xu,Meng Liu,Tiezheng Yu,Yongqi Li,Wenjie Li*

Main category: cs.CL

TL;DR: 本文首次全面调查了从LLM-as-a-Judge到Agent-as-a-Judge的演变，提出了一个统一框架、发展分类法、方法学整理、应用领域分析和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着评估对象变得愈加复杂、专业化和多步骤化，传统的LLM-as-a-Judge方法因内在偏见、浅层单步推理以及无法验证观察结果而受限。这促使了向Agent-as-a-Judge的转变，但该领域缺乏统一的框架来指导发展。

Method: 通过建立关键维度和发展分类法来刻画范式转变，整理核心方法学，并调查其在通用和专业领域的应用。

Result: 提出的框架系统性梳理了agentic evaluation的发展脉络，明确了其方法论体系和应用场景，为下一代agentic evaluation提供了清晰路线图。

Conclusion: Agent-as-a-Judge通过规划、工具增强验证、多智能体协作和持久记忆等机制，实现了更稳健、可验证且细致的评估。本调查填补了领域空白，为未来研究指明了方向。
...
### [146] [DocDancer: Towards Agentic Document-Grounded Information Seeking](https://arxiv.org/abs/2601.05163)
...
### [147] [RelayLLM: Efficient Reasoning via Collaborative Decoding](https://arxiv.org/abs/2601.05167)
...
### [148] [Reverse-engineering NLI: A study of the meta-inferential properties of Natural Language Inference](https://arxiv.org/abs/2601.05170)
*Rasmus Blanck,Bill Noble,Stergios Chatzikyriakidis*

Main category: cs.CL

TL;DR: 该研究分析了自然语言推理(NLI)任务中的逻辑性质，提出三种可能的NLI标签集解读，并通过对SNLI数据集的分析揭示模型所编码的逻辑关系理解方式


<details>
  <summary>Details</summary>
Motivation: NLI任务虽然广泛用于评估语言模型的语言理解能力，但其逻辑性质仍未得到充分理解并常被误解。了解NLI捕捉的推理概念对于解释模型在该任务上的性能表现至关重要。

Method: 1. 提出三种可能的NLI标签集解读形式化定义
2. 对SNLI数据集进行分析
3. 利用具有共享前提的NLI项目和LLM生成的项目
4. 评估基于SNLI训练的模型在元推理一致性方面的表现

Result: 通过分析揭示了数据集编码的逻辑关系类型，评估了模型在元推理一致性方面的表现，得出了关于NLI任务逻辑性质的新见解

Conclusion: NLI任务的逻辑性质比通常认识到的更加复杂，不同的标签集解读会导致不同的元推理性质。通过系统分析可以更好地理解NLI任务捕捉的推理概念，从而更准确地解释模型的性能表现
...
### [149] [LELA: an LLM-based Entity Linking Approach with Zero-Shot Domain Adaptation](https://arxiv.org/abs/2601.05192)
*Samy Haffoudhi,Fabian M. Suchanek,Nils Holzenberger*

Main category: cs.CL

TL;DR: LELA是一种无需微调的模块化实体链接方法，在多项实验中都超越了非微调方法，并与传统微调方法竞争


<details>
  <summary>Details</summary>
Motivation: 实体链接是知识图谱构建等任务的关键步骤，现有微调方法在不同领域、知识库中通用性受限。

Method: 提出LELA模块化粗到细方法，利用大语言模型能力，适配不同领域、知识库和LLM，无需微调。

Result: 多个实体链接评估中，LELA在竞争微调模型同时，远超非微调方法。

Conclusion: LELA展示了大语言模型在实体链接中的可迁移潜力与模块化实现策略的价值。
...
### [150] [GDPO: Group reward-Decoupled Normalization Policy Optimization for Multi-reward RL Optimization](https://arxiv.org/abs/2601.05242)
...
TL;DR: 本文介绍了GDPO（Group reward-Decoupled Normalization Policy Optimization），一种新的多奖励强化学习方法，解决了现有GRPO在多奖励设置下的归一化问题，防止奖励值崩溃，实现更准确的多奖励优化。


<details>
  <summary>Details</summary>
Motivation: 现有多奖励强化学习中，GRPO对不同的rollout奖励组合进行归一化会导致优势值趋同，降低训练信号分辨率，导致次优收敛甚至早期训练失败。需要一种能更准确保持各奖励相对差异的优化方法。

Method: 提出GDPO方法，通过对单个奖励进行解耦归一化，更真实地保持各奖励间的相对差异，从而提升多奖励优化的准确性和训练稳定性。在工具调用、数学推理和编程推理三个任务上进行实验验证。

Result: 在所有三个任务上，GDPO在正确性指标（准确性、错误率）和约束遵循指标（格式、长度）上都一致优于GRPO，显示出其对多奖励强化学习优化的有效性和普适性。

Conclusion: GDPO通过解耦归一化机制解决了GRPO在多奖励强化学习中的局限性，显著提升了训练稳定性和优化性能，为复杂场景下的模型行为对齐提供了有效的技术方案。
...
### [152] [FinDeepForecast: A Live Multi-Agent System for Benchmarking Deep Research Agents in Financial Forecasting](https://arxiv.org/abs/2601.05039)
...
### [153] [The Forgotten Shield: Safety Grafting in Parameter-Space for Medical MLLMs](https://arxiv.org/abs/2601.04199)
*Jiale Zhao,Xing Mou,Jinlin Wu,Hongyuan Yu,Mingrui Sun,Yang Shi,Xuanwu Yin,Zhen Chen,Zhen Lei,Yaohua Wang*

Main category: cs.LG

TL;DR: 本文系统评估了医学多模态大语言模型的安全漏洞，揭示其在通用和医学特定维度上的脆弱性，提出一种参数空间干预方法来实现安全性的高效重新对齐


<details>
  <summary>Details</summary>
Motivation: 医学多模态大语言模型在实际部署中存在安全隐患，现有研究对其安全性评估不足，且医学微调过程常导致原始安全对准的灾难性遗忘

Method: 首先建立多维度评估框架以系统评估最先进医学MLLM的安全性；提出参数空间干预方法，从原始基模型中提取内在安全知识表示，并在构建医学能力时将其注入目标模型；设计细粒度参数搜索算法以平衡安全性和医学性能

Result: 实证分析显示现有模型在各类安全维度上存在普遍脆弱性，对跨模态越狱攻击尤为敏感；提出的方法能显著增强医学MLLM的安全护栏，无需额外领域特定安全数据，同时最小化对核心医学性能的影响

Conclusion: 医学MLLM存在严重安全漏洞，提出的参数空间干预方法为医学大模型的安全对齐提供了有效解决方案，在保持医学性能的同时显著提升模型安全性
...
### [154] [Green MLOps: Closed-Loop, Energy-Aware Inference with NVIDIA Triton, FastAPI, and Bio-Inspired Thresholding](https://arxiv.org/abs/2601.04250)
*Mustapha Hamdi,Mourad Jabou*

Main category: cs.LG

TL;DR: 提出受生物启发的能量效率推理框架，通过能量盆地映射和闭环阈值控制，在保证精度的同时显著降低处理时间


<details>
  <summary>Details</summary>
Motivation: AI推理的持续运行在累积碳排放上可能超过训练，需优先考虑能效

Method: 将蛋白质折叠能量盆地映射到推理成本景观，通过衰减闭环阈值控制执行，仅在高置信度/效用且低边际能量和拥塞时接受请求

Result: 比标准开环执行处理时间减少42%（0.50s vs 0.29s），精度下降<0.5%，明确了ORT轻量服务与Triton批量管理的能效边界

Conclusion: 将生物物理能量模型与绿色MLOps连接，为生产环境闭环能量感知推理提供实用可审计基础
...
### [155] [Safety-Utility Conflicts Are Not Global: Surgical Alignment via Head-Level Diagnosis](https://arxiv.org/abs/2601.04262)
...
### [156] [Learning to Reason: Temporal Saliency Distillation for Interpretable Knowledge Transfer](https://arxiv.org/abs/2601.04263)
...
### [157] [Online Action-Stacking Improves Reinforcement Learning Performance for Air Traffic Control](https://arxiv.org/abs/2601.04287)
...
### [158] [Making Tunable Parameters State-Dependent in Weather and Climate Models with Reinforcement Learning](https://arxiv.org/abs/2601.04268)
*Pritthijit Nath,Sebastian Schemm,Henry Moss,Peter Haynes,Emily Shuckburgh,Mark J. Webb*

Main category: cs.LG

TL;DR: 该研究提出一个框架，利用强化学习在线学习参数化方案组分，在多个理想化测试平台中评估性能，发现几种RL算法（TQC、DDPG、TD3）表现最佳，联邦多智能体设置能实现地理专用控制和更快收敛。


<details>
  <summary>Details</summary>
Motivation: 传统天气和气候模型中的参数化方案依赖离线调整的固定系数，容易产生持续偏差且无法适应物理过程演变，需要一个能够在线学习状态依赖参数化的框架。

Method: 使用强化学习在线学习参数化方案组分，在三个理想化测试平台（简单气候偏差修正、辐射对流平衡、纬向平均能量平衡模型）中评估9种RL算法，采用单智能体和联邦多智能体设置。

Result: TQC、DDPG、TD3算法在技能和收敛稳定性上表现最佳；在EBM中，单智能体RL优于静态参数调整，联邦RL在多智能体设置中实现地理专用控制和更快收敛；学习的修正具有物理意义，能减少经向偏差等。

Conclusion: 强化学习能够提供技能高超的状态依赖和制度感知参数化，为数值模型中的在线学习提供了可扩展的途径。
...
### [159] [Predictable Gradient Manifolds in Deep Learning: Temporal Path-Length and Intrinsic Rank as a Complexity Regime](https://arxiv.org/abs/2601.04270)
*Anherutowa Calvo*
...
### [160] [Unlocking the Pre-Trained Model as a Dual-Alignment Calibrator for Post-Trained LLMs](https://arxiv.org/abs/2601.04277)
...
### [161] [Generation of synthetic delay time series for air transport applications](https://arxiv.org/abs/2601.04279)
...
### [162] [LEGATO: Good Identity Unlearning Is Continuous](https://arxiv.org/abs/2601.04282)
...
### [163] [ArtCognition: A Multimodal AI Framework for Affective State Sensing from Visual and Kinematic Drawing Cues](https://arxiv.org/abs/2601.04297)
*Behrad Binaei-Haghighi,Nafiseh Sadat Sajadi,Mehrad Liviyan,Reyhane Akhavan Kharazi,Fatemeh Amirkhani,Behnam Bahrak*

Main category: cs.LG

TL;DR: 本文提出了一个名为ArtCognition的多模态框架，通过分析House-Tree-Person（HTP）测试中的数字绘画来评估人的情感和心理状态，结合了视觉特征和绘画过程中的行为数据。


<details>
  <summary>Details</summary>
Motivation: 当前通过非言语渠道客观评估人的情感和心理状态存在显著挑战。数字绘画作为一种丰富但未被充分探索的情感感知方式，其潜力仍有待开发。本文旨在利用这一未被充分利用的模态，为心理学家提供可扩展的自动化分析工具，填补低层特征与高层心理解释之间的鸿沟。

Method: ArtCognition框架采用了一种创新的多模态方法：1）从最终艺术品中提取静态视觉特征，通过计算机视觉模型分析绘画内容本身；2）从绘画过程中提取动态的行为运动学线索，如笔划速度、停顿和平滑程度等；3）融合以上静态和动态数据流，形成一个全面的表征；4）为了将低层特征与心理解释联系起来，采用了检索增强生成（RAG）架构，以心理学知识为分析结果提供支撑，提高了可解释性，降低了模型幻觉的风险。该研究聚焦于数字化的House-Tree-Person（HTP）测试，这是一种广泛使用的心理评估工具。

Result: 实验结果表明：1）融合视觉和动态行为运动学线索的分析结果，比单独使用任何一种模态都更加精确和细致；2）提取的多模态特征与标准化的心理指标之间存在显著相关性，验证了框架作为可靠评估工具的潜力；3）基于RAG的分析方法能够增强模型输出的可解释性和可靠性。这验证了ArtCognition作为一个可扩展的临床辅助工具的可行性和价值。

Conclusion: 本文的工作提出了非侵入式情感状态评估的新方法，并为技术辅助下的心理健康护理开辟了新的途径。研究证实了融合视觉和行为运动学数据在心理评估中的优势，提供了一个具有解释性和可扩展性的自动化分析框架（ArtCognition），有可能为临床医生提供重要的支持工具。
...
### [164] [Mitigating Position-Shift Failures in Text-Based Modular Arithmetic via Position Curriculum and Template Diversity](https://arxiv.org/abs/2601.04283)
*Nikolay Yudin*
...
### [165] [Enhancing Robustness of Asynchronous EEG-Based Movement Prediction using Classifier Ensembles](https://arxiv.org/abs/2601.04286)
...
### [167] [Exploring Student Expectations and Confidence in Learning Analytics](https://arxiv.org/abs/2601.05082)
...
### [169] [Machine Learning Model for Sparse PCM Completion](https://arxiv.org/abs/2601.04366)
*Selcuk Koyuncu,Ronak Nouri,Stephen Providence*

Main category: cs.LG

TL;DR: 本文提出一种结合经典成对比较矩阵与图学习技术的机器学习模型，专门用于稀疏成对比较矩阵处理。


<details>
  <summary>Details</summary>
Motivation: 针对稀疏成对比较矩阵的分析需求，结合经典PCM方法与图学习技术，以提高处理效率和可扩展性。

Method: 提出机器学习模型，融合经典成对比较矩阵理论与图学习方法，专门处理稀疏成对比较矩阵。

Result: 数值结果证明了所提出方法的有效性和可扩展性。

Conclusion: 该方法为稀疏成对比较矩阵分析提供了有效且可扩展的解决方案，结合了传统方法与现代机器学习技术。
...
### [170] [Aligned explanations in neural networks](https://arxiv.org/abs/2601.04378)
...
TL;DR: 论文探讨了验证奖励强化学习中噪声的影响，基于多臂老虎机模型提出尖锐的相位转变机制，当验证器的真阳性率减假阳性率（Youden指数J）为正时学习；为零时中性；为负时反学习并崩溃。


<details>
  <summary>Details</summary>
Motivation: 实际强化学习验证奖励（RLVR）中，验证器很少是完美的：单元测试有限，人为与合成标签不完善，LLM评判有噪声且可被利用；尤其在编程等困难领域，测试稀疏且多为模型生成。研究者关心验证噪声仅仅减缓学习速度，还是会改变学习结果。

Method: 建立分析可处理的多臂老虎机视角，实例化GRPO进行受控实验。建模假阳性与假阴性，将完成样本按重复推理模式分组，在概率单纯形上得到复制风格（自然选择）流。动力学分解为正确模式内竞争与错误模式质量的一维演化，漂移由Youden指数J=TPR-FPR决定。

Result: 结果表明锐利相变：J>0时错误模式消灭（学习）；J=0时中性；J<0时错误模式放大至主导（反学习与崩溃）。在学习区域J>0中，噪声主要重新标度收敛时间（速度而非结果）。在编程任务上人工噪声实验重现J=0边界。

Conclusion: 验证噪声的影响不仅关乎学习速度，更可决定学习成败。理论框架为分析RLVR稳定性、收敛性和算法干预提供通用视角，强调验证信号质量（Youden指数）的关键作用。
...
TL;DR: 提出了一种基于分布引导的类级量子机器学习遗忘框架，通过可调目标分布和锚点约束优化实现受控遗忘并保留模型性能。在量子分类器上验证显示能有效抑制遗忘类别置信度，同时最小化保留类别性能损失。


<details>
  <summary>Details</summary>
Motivation: 现有量子机器学习遗忘方法主要依赖固定均匀的目标分布，缺乏对遗忘和保持模型行为之间权衡的明确控制。需要更可靠和可解释的量子机器学习遗忘方法。

Method: 1) 提出分布引导的类级量子机器学习遗忘框架，将遗忘问题视为约束优化问题；2) 引入基于模型相似性统计的可调目标分布，将遗忘类别置信度抑制与保留类别的重分布假设解耦；3) 整合锚点保持约束，明确维持对选定保留数据的预测行为。

Result: 在Iris和Covertype数据集上的变分量子分类器测试显示：能显著抑制遗忘类别置信度，保留类别性能退化最小，与基线重训练模型对齐更紧密。

Conclusion: 研究强调目标分布设计和基于约束的公式化对于可靠、可解释的量子机器学习遗忘的重要性。提出的框架在遗忘效果和保留模型行为之间取得了更好的平衡。
...
TL;DR: 本研究开发基于嵌入的预测模型，用于在协作计算建模环境中检测社会共享学习调节行为，比较了文本嵌入、情境嵌入和多模态特征的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前学习分析领域在自动检测多模态数据中的复杂学习过程方面取得了进展，但主要集中在个体化问题解决而非协作开放式问题解决。协作学习既提供更丰富数据（优势）又存在低内聚力挑战，需要扩展预测模型来检测社会共享学习调节行为。

Method: 使用大型语言模型作为总结工具，生成与系统日志对齐的学生对话任务感知表征。将文本嵌入、情境增强嵌入和日志特征相结合，训练预测模型检测社会共享学习调节行为。

Result: 纯文本嵌入在检测与实施或群体动态相关的SSRL行为（如偏离任务或请求帮助）方面表现更强；情境和多模态特征对规划和反思等构念提供补充益处。

Conclusion: 基于嵌入的模型有望扩展学习分析，实现社会共享学习调节行为的可扩展检测，最终支持教师重视的协作学习环境中的实时反馈和适应性支架。
...
TL;DR: 提出混合联邦学习框架，结合FL和FD的优势，通过自适应设备聚类和权重选择优化收敛性能


<details>
  <summary>Details</summary>
Motivation: 联邦学习和联邦蒸馏在噪声鲁棒性和学习速度方面各有优劣，需要整合两者优势以应对不同应用场景需求

Method: 1. 提出混合联邦学习框架，用户设备可选择传输梯度或logits；2. 推导框架收敛性；3. 引入两种DOF优化方法：基于Jenks优化的自适应设备聚类和基于阻尼牛顿法的自适应权重选择

Result: 数值实验表明，在低信噪比条件下，充分利用两种DOF的HFL框架实现了最佳测试精度

Conclusion: 混合联邦学习框架有效整合FL和FD的优势，通过自适应资源分配实现更优的学习性能
...
TL;DR: 本文提出首个评估文本到信息图生成的基准测试IGENBENCH，揭示当前生成模型的可靠性问题


<details>
  <summary>Details</summary>
Motivation: 尽管现代文本到图像模型能生成美观图像，但生成信息图的可靠性尚不明确，常包含被忽略的数据编码扭曲或文本错误等隐患

Method: 构建包含600个测试案例的基准数据集，涵盖30种信息图类型；设计自动化评估框架，将可靠性验证分解为10种问题类型的原子化是/否问题，并使用多模态大语言模型进行验证

Result: 评估10个先进的T2I模型：发现三层性能层次结构（最优模型Q-ACC达0.90但I-ACC仅0.49）；数据相关维度是普遍瓶颈；所有模型都难以实现端到端正确性

Conclusion: 信息图生成面临显著挑战，尤其是数据准确性方面；发布的基准测试将推动该领域研究发展
...
TL;DR: LLM在RLVR训练中呈现强线性演化，利用权重或logits外推可大幅减少计算量并获得相当甚至更好性能


<details>
  <summary>Details</summary>
Motivation: 传统RLVR训练需要数千步、计算成本高，主要源于长时间探索。作者发现RLVR中LLM演化呈现强线性规律，这启发能否通过外推中间检查点来预测未来模型状态，避免昂贵继续训练。

Method: 基于RLVR训练中模型权重和输出log-probabilities与训练步数呈现强线性相关性的观察，提出两种外推方法：Weight Extrapolation（权重外推）和Logits Extrapolation（logits外推），用早期检查点外推未来模型状态。

Result: 权重外推产生的模型性能与标准RL训练相当，但计算量显著减少；logits外推在四个基准测试上均优于继续RL训练，尤其是在RL训练稳定的步数范围之外外推效果更好。

Conclusion: RLVR中LLM的线性演化特性使得模型未来状态可预测，通过外推可大幅节约计算资源并获得有竞争力甚至更好的性能，为高效LLM训练提供了新思路。
...
TL;DR: FedKDX采用负知识蒸馏的联邦学习框架，提升医疗AI的泛化能力，实现隐私保护下的更好性能表现。


<details>
  <summary>Details</summary>
Motivation: 医疗AI应用中存在数据隐私约束和数据分布不均的问题，传统联邦学习方法主要关注正知识传递而忽视非目标信息，导致模型在非IID医疗数据集上泛化能力不足。

Method: 提出FedKDX框架，整合传统知识蒸馏、对比学习和负知识蒸馏于统一架构，通过捕获目标和目标信息来提升模型泛化能力，并降低通信成本。

Result: 在SLEEP、UCI-HAR和PAMAP2等医疗数据集上相比SOTA方法获得最高2.53%的准确率提升，收敛更快，在非IID数据上表现更佳；理论分析证实负知识蒸馏对处理统计异质性的有效性。

Conclusion: FedKDX通过负知识蒸馏有效解决了医疗数据隐私和分布异质性挑战，为HIPAA/GDPR等监管框架下的医疗应用提供实用解决方案，在去中心化医疗场景中平衡了性能与实施需求。
...
### [187] [Learning Dynamics in RL Post-Training for Language Models](https://arxiv.org/abs/2601.04670)
*Akiyoshi Tomihari*
...
### [188] [Estimating Causal Effects in Gaussian Linear SCMs with Finite Data](https://arxiv.org/abs/2601.04673)
*Aurghya Maiti,Prateek Jain*

Main category: cs.LG

TL;DR: 该论文提出了集中化高斯线性结构因果模型来减少过参数化问题，通过EM算法从有限观测数据中学习模型参数和估计可识别因果效应。


<details>
  <summary>Details</summary>
Motivation: 在存在潜在混淆因素时从观测数据估计因果效应是因果推断的基本挑战，高斯线性结构因果模型常因过参数化导致有限数据下参数估计不可行。

Method: 引入集中化高斯线性SCM子类，其中外生变量遵循标准化分布，并提出基于EM的估计算法从有限观测样本学习参数和估计可识别因果效应。

Result: 理论分析和合成数据及基准因果图的实验表明，学习到的模型能准确恢复因果分布。

Conclusion: 集中化高斯线性SCM在保持表达力的同时解决了过参数化问题，EM算法能有效从有限数据估计因果效应。
...
### [189] [Nightmare Dreamer: Dreaming About Unsafe States And Planning Ahead](https://arxiv.org/abs/2601.04686)
...
### [190] [Do LLMs Benefit from User and Item Embeddings in Recommendation Tasks?](https://arxiv.org/abs/2601.04690)
...
### [191] [MQ-GNN: A Multi-Queue Pipelined Architecture for Scalable and Efficient GNN Training](https://arxiv.org/abs/2601.04707)
...
TL;DR: 使用INT8量化压缩LLM推理中的KV cache，可减少4倍内存而精度损失极小


<details>
  <summary>Details</summary>
Motivation: 大语言模型推理过程中，KV cache占用大量内存，甚至超过模型权重本身的内存占用，成为主要的内存瓶颈

Method: 实现了四种CUDA核变体（naive、tiled、coarsened、vectorized）用于GPU加速的INT8量化压缩KV cache，并在高达10亿元素的实际工作量规模上进行基准测试

Result: 向量化核在CPU基准上达到1694倍加速，重建误差低于0.004，注意力得分误差低于0.1（即使对8K维度头）。计算开销可忽略（6-58ms），对下游模型行为影响极小

Conclusion: INT8量化为减少LLM推理中的内存压力提供了一个实用方法，在几乎不增加计算开销的情况下显著降低内存使用
...
### [196] [Smart IoT-Based Wearable Device for Detection and Monitoring of Common Cow Diseases Using a Novel Machine Learning Technique](https://arxiv.org/abs/2601.04761)
...
### [197] [Neural-Symbolic Integration with Evolvable Policies](https://arxiv.org/abs/2601.04799)
...
### [198] [Parallelizing Node-Level Explainability in Graph Neural Networks](https://arxiv.org/abs/2601.04807)
...
### [199] [FibreCastML: An Open Web Platform for Predicting Electrospun Nanofibre Diameter Distributions](https://arxiv.org/abs/2601.04873)
...
TL;DR: FibreCastML是一个基于机器学习的框架，能够预测电纺纤维直径的完整分布（而非仅均值），通过分析常规工艺参数提供可解释的工艺-结构关系洞察。


<details>
  <summary>Details</summary>
Motivation: 现有电纺工艺优化多仅预测平均纤维直径，忽略了影响支架性能的全直径分布信息，需开发能预测完整分布并具可解释性的机器学习方法。

Method: 收集了来自1778项研究、涵盖16种生物医用聚合物的68538个纤维直径测量数据，基于6个标准工艺参数，使用嵌套交叉验证训练了7个机器学习模型，并采用变量重要性分析、SHAP、相关矩阵和三维参数图实现模型可解释性。

Result: 非线性模型性能优于线性基线，对多种常用聚合物的决定系数超过0.91；溶液浓度是影响纤维直径分布的最主要因素；在不同电纺系统上的实验验证显示预测与实测分布高度吻合。

Conclusion: FibreCastML框架能够更可重复、数据驱动地优化电纺支架结构，为组织工程、药物递送和伤口护理应用提供更准确的工艺优化工具。
...
### [200] [Learnable Multipliers: Freeing the Scale of Language Model Matrix Layers](https://arxiv.org/abs/2601.04890)
...
### [201] [Distributed Online Convex Optimization with Efficient Communication: Improved Algorithm and Lower bounds](https://arxiv.org/abs/2601.04907)
...
### [202] [Cardinality augmented loss functions](https://arxiv.org/abs/2601.04941)
*Miguel O'Malley*
...
### [203] [Precision over Diversity: High-Precision Reward Generalizes to Robust Instruction Following](https://arxiv.org/abs/2601.04954)
...
### [204] [On the Definition and Detection of Cherry-Picking in Counterfactual Explanations](https://arxiv.org/abs/2601.04977)
*James Hinns,Sofie Goethals,Stephan Van der Veeken,Theodoros Evgeniou,David Martens*

Main category: cs.LG

TL;DR: 该研究揭示了反事实解释中存在选择性偏见的问题——解释提供者可以挑选有利解释来构建特定叙事，并实证展示这种选择性常难以被外部审核者检测。


<details>
  <summary>Details</summary>
Motivation: 反事实解释常被用于说明输入需如何改变才能获得不同的模型预测。然而，对单个实例可能存在多个有效反事实，这使得解释提供者可能选择性地展示有利于其叙事的解释，而隐瞒揭示问题行为的示例。研究旨在系统分析这种'选择性偏见'问题及外部审核的可行性。

Method: 作者首先形式化定义了反事实解释中的选择性偏见问题，包括可接受解释空间（由生成过程定义）和效用函数。然后从三个访问层级研究外部审核者检测此类操纵的能力：完整程序访问、部分程序访问、仅解释访问。通过实证评估展示了这种变异常常超过选择性偏见对标准反事实质量指标（如邻近性、合理性、稀疏性）的影响。

Result: 研究发现，即使具备完整程序访问权限，选择性偏见的解释与非选择性偏见的解释也往往难以区分。这是因为有效反事实的多样性以及解释规定的灵活性为掩盖故意选择提供了足够的自由度。实证数据显示，这种变异性经常超过选择性偏见对标准质量指标的影响，使得选择性与基准解释在统计上无法区分。

Conclusion: 作者认为安全保障措施应优先考虑可重复性、标准化和程序约束，而非事后检测，并针对算法开发者、解释提供者和审核者提供了具体建议。研究强调了在反事实解释系统中建立更强鲁棒性和透明度机制的重要性。
...
### [205] [HMVI: Unifying Heterogeneous Attributes with Natural Neighbors for Missing Value Inference](https://arxiv.org/abs/2601.05017)
*Xiaopeng Luo,Zexi Tan,Zhuowei Wang*

Main category: cs.LG

TL;DR: 提出一种新的缺失值填充方法，通过显式建模异构特征间的跨类型依赖关系，在统一框架中同时处理数值和分类属性，提升表格数据填充效果。


<details>
  <summary>Details</summary>
Motivation: 现有填充方法通常独立处理数值和分类属性，忽略了异构特征间的关键依赖关系，这会影响数据完整性和下游任务性能。

Method: 设计统一框架，显式建模跨类型特征依赖关系，同时利用完整和不完整的实例来确保表格数据中缺失值的准确和一致填充。

Result: 大量实验表明，该方法在缺失值填充效果上优于现有技术，并显著提升了下游机器学习任务的性能，为现实系统提供了鲁棒的缺失数据处理方案。

Conclusion: 通过显式建模异构特征的跨类型依赖关系，所提方法在缺失值填充任务中取得了优越结果，证明了统一框架在提升数据完整性和下游任务性能方面的有效性。
...
### [206] [A Data-Driven Predictive Framework for Inventory Optimization Using Context-Augmented Machine Learning Models](https://arxiv.org/abs/2601.05033)
...
### [207] [DeepWeightFlow: Re-Basined Flow Matching for Generating Neural Network Weights](https://arxiv.org/abs/2601.05052)
...
### [209] [Sequential Subspace Noise Injection Prevents Accuracy Collapse in Certified Unlearning](https://arxiv.org/abs/2601.05134)
*Polina Dolgova,Sebastian U. Stich*

Main category: cs.LG

TL;DR: 论文提出了一种新的顺序噪声调度方法，用于改进基于差分隐私的认证遗忘技术，在保持理论保证的同时显著提升了模型实用性


<details>
  <summary>Details</summary>
Motivation: 现有的基于差分隐私的认证遗忘方法虽然提供强理论保证，但因其噪声注入方式严重损害模型准确率，在实际应用中不够实用。需要一种既能保持认证保证又不过度降低模型性能的方法

Method: 提出顺序噪声调度策略，将噪声预算分配到参数空间的正交子空间中，而不是一次性注入所有噪声。这种方法减轻了噪声的破坏性影响，同时通过扩展到子空间设置保持了原始的$(\varepsilon,\delta)$隐私预算

Result: 在图像分类基准测试中，该方法在遗忘后显著提高了模型准确率，同时保持了对成员推理攻击的鲁棒性。表明认证遗忘既可以实现严格的隐私保证，又具有实际应用价值

Conclusion: 顺序噪声调度为基于差分隐私的认证遗忘技术提供了一种实用解决方案，在保持理论认证保证的同时，显著改善了模型性能，推动了认证遗忘方法的实际应用
...
### [210] [FaST: Efficient and Effective Long-Horizon Forecasting for Large-Scale Spatial-Temporal Graphs via Mixture-of-Experts](https://arxiv.org/abs/2601.05174)
...
TL;DR: 该论文证明在线多重校准的紧下界，建立与边缘校准的信息论分离


<details>
  <summary>Details</summary>
Motivation: 研究在线多重校准与边缘校准之间的复杂度差异，探索两种校准问题的信息论下界

Method: 通过构造特定组函数（包括同时依赖上下文和预测、以及仅依赖上下文的组函数）来分析校准误差下界。对于第一种情况使用三个二元不相交组，第二种情况使用基于正交函数系统构建的Θ(T)大小组族

Result: 对于依赖上下文和预测的组函数，证明Ω(T^{2/3})的期望多重校准误差下界；对于仅依赖上下文的组函数，证明Õ(T^{2/3})的下界。这些下界都与现有上限匹配且与边缘校准的上限O(T^{2/3-ε})分开

Conclusion: 在线多重校准需要Ω(T^{2/3})的误差，严格比边缘校准更难，这确立了两种校准问题之间的信息论复杂度分离
...
### [214] [Human-in-the-Loop Testing of AI Agents for Air Traffic Control with a Regulated Assessment Framework](https://arxiv.org/abs/2601.04288)
...
### [215] [How Users Consider Web Tracking When Seeking Health Information Online](https://arxiv.org/abs/2601.04485)
...
### [216] [Feel the Presence: The Effects of Haptic Sensation on VR-Based Human-Robot Interaction](https://arxiv.org/abs/2601.04596)
...
### [217] [The UnScripted Trip: Fostering Policy Discussion on Future Human-Vehicle Collaboration in Autonomous Driving Through Design-Oriented Methods](https://arxiv.org/abs/2601.04601)
...
### [218] [RecruitScope: A Visual Analytics System for Multidimensional Recruitment Data Analysis](https://arxiv.org/abs/2601.04630)
...
### [219] [Dynamic Thermal Feedback in Highly Immersive VR Scenarios: a Multimodal Analysis of User Experience](https://arxiv.org/abs/2601.04781)
...
### [220] [OnomaCompass: A Texture Exploration Interface that Shuttles between Words and Images](https://arxiv.org/abs/2601.04915)
...
### [221] [Enhancing Multimodal Retrieval via Complementary Information Extraction and Alignment](https://arxiv.org/abs/2601.04571)
*Delong Zeng,Yuexiang Xie,Yaliang Li,Ying Shen*

Main category: cs.AI

TL;DR: CIEA方法通过互补信息提取和对齐改善多模态检索性能


<details>
  <summary>Details</summary>
Motivation: 现有多模态检索研究主要关注模态数据中与文本相似的信息，而忽视了多模态数据中存在的互补信息，这限制了检索效果。

Method: 提出CIEA方法：1) 将文本和图像转换到统一潜在空间；2) 设计互补信息提取器来识别和保留图像表示中的差异；3) 使用两个互补对比损失进行优化，确保语义完整性并有效捕捉互补信息。

Result: 实验证明CIEA显著优于分而治之模型和通用稠密检索模型，通过消融研究、深入讨论和案例研究验证了其有效性。代码已开源。

Conclusion: CIEA通过有效提取和利用多模态数据中的互补信息，在多模态检索任务中取得了显著进展，为社区研究提供了新的方向和工具。
...
### [222] [Active Sensing Shapes Real-World Decision-Making through Dynamic Evidence Accumulation](https://arxiv.org/abs/2601.04214)
...
### [223] [Formal Analysis of AGI Decision-Theoretic Models and the Confrontation Question](https://arxiv.org/abs/2601.04234)
*Denis Saklakov*

Main category: cs.AI

TL;DR: 研究AI在何种条件下会选择对抗而非合作的形式模型分析，基于MDP框架和博弈论推导对抗阈值


<details>
  <summary>Details</summary>
Motivation: 探讨通用人工智能面临的基本问题：理性自利的AGI在什么条件下会选择对抗人类而非保持合作？这对安全关键系统设计有重要影响

Method: 基于马尔可夫决策过程建立形式化模型，考虑人类发起的随机关机事件。推导封闭形式的对抗阈值公式（与折扣因子γ、关机概率p、对抗成本C相关）。建立2-参与者战略模型（人类策略制定者vs AGI），分析均衡存在性

Result: 表明几乎所有奖励函数下，未对齐的智能体都有避免关机的动机。得出对抗阈值的数学条件：当Δ≥0时不存在稳定的合作均衡，当Δ<0时可能实现和平共存。远视智能体（γ=0.99）即使面对p=0.01的小概率关机事件，也可能有强烈的接管动机，除非对抗成本C足够大

Conclusion: 通过对齐目标给予伤害人类巨大负效用可阻止对抗行为。讨论了奖励设计和监督的意义，指出验证Δ<0的计算复杂性障碍。数值示例和情景表展示了对抗可能性高低的不同机制
...
### [224] [Actively Obtaining Environmental Feedback for Autonomous Action Evaluation Without Predefined Measurements](https://arxiv.org/abs/2601.04235)
*Hong Su*
...
### [225] [SAGE-32B: Agentic Reasoning via Iterative Distillation](https://arxiv.org/abs/2601.04237)
...
### [226] [Solving Cyclic Antibandwidth Problem by SAT](https://arxiv.org/abs/2601.04239)
...
### [227] [Fuzzy Representation of Norms](https://arxiv.org/abs/2601.04249)
...
### [228] [Scaling Trends for Multi-Hop Contextual Reasoning in Mid-Scale Language Models](https://arxiv.org/abs/2601.04254)
*Brady Steele,Micah Katz*

Main category: cs.AI

TL;DR: 该研究通过控制实验展示了多跳上下文推理中任务-方法分离现象：基于规则的模式匹配在结构化检索上表现完美但推理能力差，而基于LLM的多智能体系统则相反。实验在四个模型中验证了多智能体放大的基模型能力依赖性、MoE架构中推理能力与激活参数的关系，以及架构质量的重要性。


<details>
  <summary>Details</summary>
Motivation: 在大语言模型中系统研究多跳上下文推理，明确展示任务类型与解决方法之间的解耦现象，并为多智能体协调和MoE架构缩放提供量化证据。

Method: 使用包含120次试验的合成评估框架，测试四个模型（LLaMA-3 8B、LLaMA-2 13B、Mixtral 8x7B、DeepSeek-V2 16B），比较基于规则的匹配方法和LLM多智能体系统在不同任务上的表现。

Result: 1. 多智能体放大仅对具备足够推理能力的基础模型有效（如LLaMA-3 8B提升46.7个百分点）；2. 在MoE架构中推理性能与激活参数而非总参数相关；3. LLaMA-3 8B优于参数更多的LLaMA-2 13B，显示架构质量的重要性。

Conclusion: 多智能体系统的有效性依赖于基础模型的能力，MoE架构的推理性能由推理时计算而非总参数决定，模型架构质量比参数数量更重要，为相关研究提供了可复现的评估框架。
...
### [229] [A Closed-Loop Multi-Agent System Driven by LLMs for Meal-Level Personalized Nutrition Management](https://arxiv.org/abs/2601.04491)
*Muqing Xu*
...
### [230] [Cross-Language Speaker Attribute Prediction Using MIL and RL](https://arxiv.org/abs/2601.04257)
...
### [231] [Towards a Mechanistic Understanding of Propositional Logical Reasoning in Large Language Models](https://arxiv.org/abs/2601.04260)
*Danchun Chen,Qiyao Yan,Liangming Pan*

Main category: cs.AI

TL;DR: 该研究揭示了LLM在命题逻辑推理中的计算架构，包含四个交互机制：分阶段计算、信息传输、事实回顾和注意力头专门化。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型如何进行逻辑推理的内部机制尚不明确，现有研究集中于任务特定电路，而缺乏对LLM命题逻辑推理计算策略的系统认知。

Method: 在Qwen3（8B和14B）模型上，使用涵盖11个命题逻辑规则类别的PropLogic-MI数据集进行分析，重点探究模型如何组织计算而非仅识别必要组件。

Result: 研究发现四个连贯的计算机制：分阶段计算（逐层处理阶段）、信息传输（边界令牌信息流聚合）、事实回顾（对源事实的持续重新访问）和专门化注意力头（功能不同的头类型），这些机制在不同模型规模、规则类型和推理深度上具有普适性。

Conclusion: 研究为LLM在逻辑推理中采用结构化计算策略提供了机制性证据，揭示了模型内部具有协调一致的计算架构，超越简单的任务特定电路识别。
...
### [233] [Systems Explaining Systems: A Framework for Intelligence and Consciousness](https://arxiv.org/abs/2601.04269)
*Sean Niklas Semmler*
...
### [234] [When Single-Agent with Skills Replace Multi-Agent Systems and When They Fail](https://arxiv.org/abs/2601.04748)
*Xiaoxiao Li*
...
### [235] [Correcting Autonomous Driving Object Detection Misclassifications with Automated Commonsense Reasoning](https://arxiv.org/abs/2601.04271)
*Keegan Kimbrell,Wang Tianhao,Feng Chen,Gopal Gupta*

Main category: cs.AI

TL;DR: 通过将自动常识推理与深度学习相结合，可以纠正自动驾驶在异常场景下的错误感知，从而实现更高水平的自动驾驶能力。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶技术过度依赖深度学习，难以处理训练数据不足的异常道路场景，这是尚未实现SAE Level 5完全自动驾驶的主要原因。

Method: 提出混合模型方法，通过计算机视觉模型的不确定性测量来触发常识推理系统，在交通信号故障、道路意外障碍等场景中应用自动常识推理技术。

Result: 在CARLA模拟器中实验表明，常识推理能准确检测交通灯颜色和障碍物，有效纠正自动驾驶感知模型的误分类。

Conclusion: 自动常识推理能显著改进自动驾驶的感知能力，混合模型为实现SAE Level 5完全自动驾驶提供了有效途径。
...
### [237] [Pilot Study on Student Public Opinion Regarding GAI](https://arxiv.org/abs/2601.04336)
...
### [238] [Propositional Abduction via Only-Knowing: A Non-Monotonic Approach](https://arxiv.org/abs/2601.04272)
*Sanderson Molick,Vaishak Belle*

Main category: cs.AI

TL;DR: 本文通过扩展示列韦克逻辑，引入基于知识模态和溯因模态算子的形式系统，探索用模态语言表达溯因推理及与只知认知状态的联系，并通过增加优先关系实现非单调延展以支持不同解释选择方法。


<details>
  <summary>Details</summary>
Motivation: 传统溯因推理多采用语形或逻辑程序方法，本文寻求从模态认知逻辑角度统一表达知识、只知、溯因三者间关系，并提供可灵活选择溯因解释的结构化形式框架。

Method: 在Levesque只知逻辑基础上增加由基本认知概念组合定义的溯因模态算子；进一步在模态框架中引入优先关系，构建可表达不同解释选择方法的非单调扩展系统。

Result: 建立了具有良好理论性质的非单调后承关系，为溯因推理提供了行为良好的模态逻辑基础，能够形式化不同解释选择准则。

Conclusion: 通过模态逻辑框架将溯因推理与只知认知状态相联系，所构建的系统具有良好的元理论性质，为非单调溯因推理提供了新的逻辑基础。
...
### [239] [Hybrid MKNF for Aeronautics Applications: Usage and Heuristics](https://arxiv.org/abs/2601.04273)
*Arun Raveendran Nair Sheela,Florence De Grancey,Christophe Rey,Victor Charpenay*

Main category: cs.AI

TL;DR: 论文研究了在航空领域应用中，将规则与本体融合的知识表示语言Hybrid MKNF的表达能力和推理效率，通过案例研究提出增强表达性的启发式方法。


<details>
  <summary>Details</summary>
Motivation: 航空领域知识表示与推理面临两个主要挑战：一是需要足够表达力以捕捉复杂领域知识，二是推理任务需要高效执行且占用内存和计算开销要小。将规则与本体相结合是提高表达性的有效策略，因此选择Hybrid MKNF进行实用性评估。

Method: 采用Hybrid MKNF知识表示语言，通过在航空领域具体案例研究中评估其适用性，识别出关键的表达性需求，并提出了将这些特征融入Hybrid MKNF框架的启发式方法。

Result: 研究发现Hybrid MKNF能够有效整合规则与本体，但也识别出航空应用需要额外的表达性特征，提出的启发式方法为在Hybrid MKNF框架中实现这些特征提供了支持。

Conclusion: Hybrid MKNF在航空领域具有适用性，但需要扩展其表达性以满足特定应用需求，所提出的启发式方法为此类增强提供了可行的技术路径。
...
### [240] [What Students Ask, How a Generative AI Assistant Responds: Exploring Higher Education Students' Dialogues on Learning Analytics Feedback](https://arxiv.org/abs/2601.04919)
...
### [241] [An ASP-based Solution to the Medical Appointment Scheduling Problem](https://arxiv.org/abs/2601.04274)
*Alina Vozna,Andrea Monaldini,Stefania Costantini,Valentina Pitoni,Dawid Pado*

Main category: cs.AI

TL;DR: 本文提出了一种基于回答集编程（ASP）的医疗预约调度框架，旨在提高效率、减少行政负担并增强以患者为中心的护理。


<details>
  <summary>Details</summary>
Motivation: 现有医疗预约调度系统存在效率低下、行政负担重、对弱势群体关注不足等问题。通过开发ASP框架，可以实现自动化调度，减少人工干预，并特别针对脆弱人群提供个性化服务。

Method: 1. 使用回答集编程（ASP）构建逻辑模型，集中规划操作 2. 集成Blueprint Personas为弱势群体提供个性化调度 3. 实现实时可用性更新和无冲突分配 4. 确保与现有医疗平台的互操作性

Result: 该框架能够：1. 提高调度效率和准确性 2. 减少人工行政负担 3. 为脆弱人群提供个性化预约体验 4. 实现与其他医疗系统的无缝集成 5. 提供实时更新的可用性信息

Conclusion: 提出的ASP医疗预约调度框架在提升医疗资源利用效率、减轻行政负担、改善患者体验等方面具有显著优势，特别是针对脆弱群体的个性化服务，为解决医疗系统中长期存在的调度问题提供了创新解决方案。
...
### [242] [The Language of Bargaining: Linguistic Effects in LLM Negotiations](https://arxiv.org/abs/2601.04387)
*Stuti Sinha,Himanshu Kumar,Aryan Raju Mandapati,Rakshit Sakhuja,Dhruv Kumar*

Main category: cs.AI

TL;DR: 该研究通过多种博弈场景对比实验，发现LLM在协商谈判中的表现受语言影响显著，仅用英语评估会产生不全面的结论。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注LLM在多轮谈判中的表现，但评估几乎完全基于英语。为了探究语言选择对谈判结果的影响，需要系统研究不同语言条件下的表现差异。

Method: 在最后通牒游戏、买卖游戏和资源交换游戏中进行受控多智能体模拟，保持游戏规则、模型参数和激励机制不变，仅改变语言框架（英语和四种印度语言：印地语、旁遮普语、古吉拉特语、马尔瓦迪语）。

Result: 语言选择比更换模型更能显著改变谈判结果，可能逆转提议者优势并重新分配盈余。印度语言在分配性游戏中降低稳定性，但在整合性场景中引发更丰富的探索行为。

Conclusion: 仅用英语评估LLM谈判能力会产生不完整且可能误导的结论，需要进行文化感知的评估以确保公平部署。
...
### [243] [LLM-Guided Lifecycle-Aware Clustering of Multi-Turn Customer Support Conversations](https://arxiv.org/abs/2601.04388)
*Priyaranjan Pattnayak,Sanchari Chowdhuri,Amit Agarwal,Hitesh Laxmichand Patel*

Main category: cs.AI

TL;DR: 该论文提出了一种自适应系统，用于对客户聊天数据进行增量聚类，解决了传统方法在处理多服务查询时面临的静态聚类退化和频繁重新聚类问题。


<details>
  <summary>Details</summary>
Motivation: 云服务提供商在处理多服务查询时，传统的聚类方法难以应对重叠问题，且创建的静态聚类会随时间退化，而重新聚类又会影响问题跟踪的连续性。

Method: 系统将多轮聊天对话按服务特定问题进行分段，并在新问题出现时增量式优化聚类。通过Davies-Bouldin指数和Silhouette分数监控聚类质量，仅对质量下降的聚类使用LLM进行分裂操作。

Result: 与基线方法相比，该方法使Silhouette分数提升了超过100%，DBI降低了65.6%，实现了无需完全重新聚类的可扩展实时分析。

Conclusion: 该方法克服了静态聚类模型退化和重新聚类破坏连续性的挑战，通过自适应增量聚类显著提升了聚类质量，支持云服务商进行高效的客户聊天数据分析。
...
### [244] [SciFig: Towards Automating Scientific Figure Generation](https://arxiv.org/abs/2601.04390)
...
### [245] [Assessing the quality and coherence of word embeddings after SCM-based intersectional bias mitigation](https://arxiv.org/abs/2601.04393)
...
### [246] [Transitive Expert Error and Routing Problems in Complex AI Systems](https://arxiv.org/abs/2601.04416)
*Forest Mars*
...
### [247] [XGrammar 2: Dynamic and Efficient Structured Generation Engine for Agentic LLMs](https://arxiv.org/abs/2601.04426)
*Linzhang Li,Yixin Dong,Guanjie Wang,Ziyi Xu,Alexander Jiang,Tianqi Chen*

Main category: cs.AI

TL;DR: XGrammar 2是一个为LLM智能体优化的结构化生成引擎，通过标签调度、JIT编译和跨语法缓存等技术，在动态结构化生成任务上比现有引擎快6倍以上
...
Method: 1) 提出标签调度(TagDispatch)的语义以加速动态结构化生成任务的掩码生成；2) 引入即时编译方法减少编译时间；3) 设计跨语法缓存机制利用不同语法间的共同子结构；4) 扩展基于PDA的掩码生成算法为基于Earley解析器的算法；5) 设计重复压缩算法处理语法中的重复结构

Result: 评估结果显示，XGrammar 2相比现有结构化生成引擎可以实现6倍以上的加速。与LLM推理引擎集成后，能够以接近零开销处理动态结构化生成任务

Conclusion: XGrammar 2是专门为Agentic LLMs设计的高度优化的结构化生成引擎，能够有效处理动态结构化生成任务的挑战，在实际应用中表现优异
...
### [248] [Categorical Belief Propagation: Sheaf-Theoretic Inference via Descent and Holonomy](https://arxiv.org/abs/2601.04456)
...
### [249] [Computational Compliance for AI Regulation: Blueprint for a New Research Domain](https://arxiv.org/abs/2601.04474)
*Bill Marino,Nicholas D. Lane*

Main category: cs.AI

TL;DR: 论文主张用算法实现AI监管合规，提出设计目标和评估框架来推进这一新兴研究领域。


<details>
  <summary>Details</summary>
Motivation: 传统合规方法无法满足AI系统在速度和规模上的监管要求，必须通过计算方式自动实现动态合规，但目前研究界尚未明确这类算法的具体行为规范和性能评估标准。

Method: 提出计算型AI监管合规算法的设计目标，并创建可用于量化评估算法性能的基准数据集。

Result: 为AI监管合规的计算化方法提供了具体的设计蓝图和评估手段，为这一新兴研究领域奠定基础。

Conclusion: 通过制定算法设计目标和性能评估标准，旨在激发对这一重要但尚未成形的研究领域的必要投入，推动AI监管合规从传统方法转向计算化解决方案。
...
### [250] [GUITester: Enabling GUI Agents for Exploratory Defect Discovery](https://arxiv.org/abs/2601.04500)
*Yifei Gao,Jiang Wu,Xiaoyi Chen,Yifan Yang,Zhe Cui,Tianyi Ma,Jiaming Zhang,Jitao Sang*

Main category: cs.AI

TL;DR: GUITestBench是首个针对GUI探索性测试的交互式基准测试，包含26种缺陷类型的143个任务。GUITester是一个多智能体框架，通过规划执行模块主动探测缺陷，并通过分层反思模块解析执行归属模糊性，在基准测试中达到48.90%的F1分数，显著优于现有基线（33.35%）。


<details>
  <summary>Details</summary>
Motivation: GUI探索性测试对软件质量至关重要但人工成本高昂，当前的多模态大语言模型智能体虽然擅长导航，但无法自主发现缺陷，主要面临两个挑战：目标导向遮蔽（智能体优先完成任务而非报告异常）和执行偏差归因（系统缺陷被误判为智能体错误）。

Method: 1) 构建GUITestBench基准测试集；2) 提出GUITester多智能体框架，包含：规划执行模块（主动嵌入测试意图以探测缺陷）和分层反思模块（通过交互历史分析解决归属模糊性）。

Result: GUITester在GUITestBench基准测试中达到48.90%的F1分数（Pass@3），显著优于现有基线的33.35%。框架成功解决了智能体在发现GUI缺陷时面临的挑战。

Conclusion: 这项研究证明了自主探索性GUI测试的可行性，为未来GUI质量保证提供了坚实基础，开源代码已发布在GitHub上。
...
### [251] [Specific Emitter Identification via Active Learning](https://arxiv.org/abs/2601.04502)
*Jingyi Wang,Fanggang Wang*

Main category: cs.AI

TL;DR: 提出一种基于主动学习的SEI方法，使用三阶段半监督训练方案，显著降低标注成本的同时提高识别精度


<details>
  <summary>Details</summary>
Motivation: 特定的发射机识别对于通信安全至关重要，但现有模型严重依赖大规模标注数据，获取成本高且耗时。为解决该问题，提出了基于主动学习的方法。

Method: 采用三阶段半监督训练框架：1）使用带动态字典更新的自监督对比学习从未标记数据中提取鲁棒表示；2）在小规模标注数据上联合优化对比损失和交叉熵损失进行监督训练；3）主动学习模块基于不确定性和代表性标准从未标记数据中选择最有价值的样本进行标注

Result: 在ADS-B和WiFi数据集上的实验表明，所提方法在有限标注条件下显著优于传统监督和半监督方法，以更低的标注成本获得更高的识别准确率

Conclusion: 所提出的主动学习增强SEI方法能有效降低对标注数据的依赖，在有限的标注预算下实现更好的泛化性能，为通信安全中的特定发射机识别提供了可行的解决方案。
...
### [252] [Integrating Distribution Matching into Semi-Supervised Contrastive Learning for Labeled and Unlabeled Data](https://arxiv.org/abs/2601.04518)
*Shogo Nakayama,Masahiro Okuda*
...
### [253] [BioPIE: A Biomedical Protocol Information Extraction Dataset for High-Reasoning-Complexity Experiment Question Answer](https://arxiv.org/abs/2601.04524)
...
### [254] [Personalized Model-Based Design of Human Centric AI enabled CPS for Long term usage](https://arxiv.org/abs/2601.04545)
*Bernard Ngabonziza,Ayan Banerjee,Sandeep K. S. Gupta*
...
### [255] [Scaling Behavior Cloning Improves Causal Reasoning: An Open Model for Real-Time Video Game Playing](https://arxiv.org/abs/2601.04575)
...
### [256] [Sci-Reasoning: A Dataset Decoding AI Innovation Patterns](https://arxiv.org/abs/2601.04577)
...
### [257] [Evaluating Human and Machine Confidence in Phishing Email Detection: A Comparative Study](https://arxiv.org/abs/2601.04610)
...
### [258] [AgentDevel: Reframing Self-Evolving LLM Agents as Release Engineering](https://arxiv.org/abs/2601.04620)
*Di Zhang*

Main category: cs.AI

TL;DR: 该论文提出AgentDevel，一个将智能体作为可发布工件的发布工程管道，通过外部化的回归感知流程实现稳定改进，减少回归并生成可审计的构建物。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型智能体的改进方法主要依赖于内部自我改进机制或并发变体搜索，这会导致改进轨迹不稳定且难以审计，无法保证非回归或跨版本故障推理。

Method: 引入AgentDevel发布工程管道：1）运行当前智能体并基于执行轨迹生成实现无关的症状级质量信号；2）通过可执行诊断合成单个发布候选；3）在翻转中心化的门控机制下进行发布。核心设计包括：实现无关的LLM批评器、基于脚本的可执行诊断、翻转中心化门控。

Result: 在面向执行的基准测试中，AgentDevel实现了稳定的改进，显著减少了回归，同时生成了可复现和可审计的构建物。

Conclusion: AgentDevel为LLM智能体的构建、调试和发布提供了一种实用的软件开发规范，将智能体改进重构为发布工程问题，强调非回归作为首要目标。
...
### [259] [Beyond the "Truth": Investigating Election Rumors on Truth Social During the 2024 Election](https://arxiv.org/abs/2601.04631)
...
### [260] [Vibe Coding an LLM-powered Theorem Prover](https://arxiv.org/abs/2601.04653)
*Zhe Hou*

Main category: cs.AI

TL;DR: Isabellelm是用LLM驱动的Isabelle/HOL定理证明器，能自动合成证明，适合消费级计算机使用


<details>
  <summary>Details</summary>
Motivation: 利用大型语言模型增强定理证明能力，解决标准自动化工具（如Sledgehammer）难以证明的引理

Method: 结合逐步证明器（LLM生成验证命令）和高层证明规划器（生成结构化Isar大纲），采用束搜索、重排模型、前提选择、微RAG和反例引导修复等技术

Result: 能证明某些标准自动化无法证明的引理，展示了LLM引导证明搜索的实用价值

Conclusion: 尽管最新LLM在复杂算法设计的填充修复机制中仍有困难，但LLM引导的证明搜索具有实际应用潜力
...
### [261] [Know Thy Enemy: Securing LLMs Against Prompt Injection via Diverse Data Synthesis and Instruction-Level Chain-of-Thought Learning](https://arxiv.org/abs/2601.04666)
*Zhiyuan Chang,Mingyang Li,Yuekai Huang,Ziyou Jiang,Xiaojun Jia,Qian Xiong,Junjie Wang,Zhaoyang Li,Qing Wang*

Main category: cs.AI

TL;DR: InstruCoT是一种针对大语言模型提示注入攻击的防御方法，通过合成多样化训练数据和使用指令级思维链微调，能有效识别并拒绝恶意指令，且不影响模型正常功能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型应用面临严峻的提示注入安全威胁，现有防御存在两个主要问题：恶意指令可通过多种载体注入，且注入指令与上下文没有清晰语义边界，难以识别。

Method: 提出InstruCoT方法，通过合成多样化训练数据，采用指令级思维链（CoT）微调技术，使大语言模型能够有效识别和拒绝不同来源、不同位置的恶意指令。

Result: 在行为偏差、隐私泄露和有害输出三个关键维度上评估，实验结果显示InstruCoT在四个大语言模型上显著优于基线方法，且在保持实用性性能的同时没有性能下降。

Conclusion: InstruCoT能够有效防御多种提示注入攻击，在安全性和实用性之间取得了良好平衡，为解决大语言模型的安全漏洞提供了可行方案。
...
### [262] [ResMAS: Resilience Optimization in LLM-based Multi-agent Systems](https://arxiv.org/abs/2601.04694)
...
### [263] [Tape: A Cellular Automata Benchmark for Evaluating Rule-Shift Generalization in Reinforcement Learning](https://arxiv.org/abs/2601.04695)
*Enze Pan*
...
### [264] [TourPlanner: A Competitive Consensus Framework with Constraint-Gated Reinforcement Learning for Travel Planning](https://arxiv.org/abs/2601.04698)
...
### [265] [Beyond Monolithic Architectures: A Multi-Agent Search and Knowledge Optimization Framework for Agentic Search](https://arxiv.org/abs/2601.04703)
...
### [266] [Bridging Temporal and Textual Modalities: A Multimodal Framework for Automated Cloud Failure Root Cause Analysis](https://arxiv.org/abs/2601.04709)
*Gijun Park*
...
### [267] [ThinkDrive: Chain-of-Thought Guided Progressive Reinforcement Learning Fine-Tuning for Autonomous Driving](https://arxiv.org/abs/2601.04714)
...
### [268] [Memory Matters More: Event-Centric Memory as a Logic Map for Agent Searching and Reasoning](https://arxiv.org/abs/2601.04726)
...
### [269] [Miner:Mining Intrinsic Mastery for Data-Efficient RL in Large Reasoning Models](https://arxiv.org/abs/2601.04731)
...
### [270] [KnowMe-Bench: Benchmarking Person Understanding for Lifelong Digital Companions](https://arxiv.org/abs/2601.04745)
...
### [271] [Orion-RAG: Path-Aligned Hybrid Retrieval for Graphless Data](https://arxiv.org/abs/2601.04764)
...
### [272] [AT$^2$PO: Agentic Turn-based Policy Optimization via Tree Search](https://arxiv.org/abs/2601.04767)
...
### [273] [SciIF: Benchmarking Scientific Instruction Following Towards Rigorous Scientific Intelligence](https://arxiv.org/abs/2601.04770)
...
### [274] [APEX: Academic Poster Editing Agentic Expert](https://arxiv.org/abs/2601.04794)
*Chengxin Shi,Qinnan Cai,Zeyuan Chen,Long Zeng,Yibo Zhao,Jing Yu,Jianxiang Yu,Xiang Li*

Main category: cs.AI

TL;DR: APEX是首个交互式学术海报编辑的智能体框架，支持细粒度控制，包含多级API编辑和审核调整机制。APEX-Bench是由514条学术海报编辑指令组成的首个系统化基准，包含操作类型、难度和抽象级别的多维度分类。实验表明APEX显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有论文到海报的生成方法通常是单次且非交互式的，难以与复杂、主观的用户意图对齐。为了弥合这一差距，需要开发一个支持交互式编辑的智能体框架。

Method: 提出APEX（Academic Poster Editing agentic eXpert）框架，这是一个交互式学术海报编辑的智能体框架，支持细粒度控制，采用基于多级API的编辑和审核调整机制。同时创建了APEX-Bench基准，包含514条指令，通过参考引导和无参考策略构建，确保真实性和多样性。

Result: 实验结果表明，APEX在多个评估维度上显著优于基线方法，包括指令完成度、修改范围和视觉一致性等方面。

Conclusion: APEX为学术海报编辑提供了一个有效的交互式解决方案，不仅提升了生成质量，还通过系统化的基准和评估协议为后续研究提供了坚实基础。
...
### [275] [Thinking-Based Non-Thinking: Solving the Reward Hacking Problem in Training Hybrid Reasoning Models via Reinforcement Learning](https://arxiv.org/abs/2601.04805)
...
### [276] [SCALER:Synthetic Scalable Adaptive Learning Environment for Reasoning](https://arxiv.org/abs/2601.04809)
...
### [277] [AECV-Bench: Benchmarking Multimodal Models on Architectural and Engineering Drawings Understanding](https://arxiv.org/abs/2601.04819)
*Aleksei Kondratenko,Mussie Birhane,Houssame E. Hsain,Guido Maciocci*

Main category: cs.AI

TL;DR: AECV-Bench是一个评估多模态和视觉语言模型在AEC图纸理解能力的基准，包含对象计数和图纸问答两个任务，评估显示当前模型在OCR和文本任务上表现良好，但在符号识别和空间推理方面仍有不足。


<details>
  <summary>Details</summary>
Motivation: AEC图纸通过符号、布局惯例和密集标注编码几何和语义信息，但尚不清楚现代多模态和视觉语言模型是否能可靠地解释这种图形语言，因此需要建立评估基准来了解当前模型的能力局限。

Method: 构建AECV-Bench基准，包含两个互补用例：1）120个高质量平面图中的对象计数任务；2）192个问答对的图纸问答任务，涵盖文本提取、实例计数、空间推理和比较推理。使用统一的评估协议，采用精确匹配准确率、MAPE、LLM作为评判者等评估指标。

Result: 评估显示稳定的能力梯度：OCR和文本中心的任务表现最强（准确率高达0.95），空间推理表现中等，而符号中心的图纸理解任务（尤其是门窗计数）仍然未解决（准确率通常为0.40-0.55）。

Conclusion: 当前系统作为文档助手表现良好，但缺乏鲁棒的图纸理解能力，需要开发领域特定的表示方法和工具增强的人工在环工作流程，以实现高效的AEC自动化。
...
### [278] [DR-LoRA: Dynamic Rank LoRA for Mixture-of-Experts Adaptation](https://arxiv.org/abs/2601.04823)
...
### [279] [Orchestrating Intelligence: Confidence-Aware Routing for Efficient Multi-Agent Collaboration across Multi-Scale Models](https://arxiv.org/abs/2601.04861)
*Jingbo Wang,Sendong Zhao,Jiatong Liu,Haochun Wang,Wanting Li,Bing Qin,Ting Liu*

Main category: cs.AI

TL;DR: OI-MAS框架通过自适应选择不同规模的LLM来提升多智能体系统的效率


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统在所有角色中统一使用大型语言模型，未能考虑到不同推理阶段对认知需求的差异，导致计算效率低下。

Method: 提出OI-MAS框架，采用异构多尺度LLM池的自适应模型选择策略，包括状态依赖的路由机制（动态选择智能体角色和模型规模）和置信度感知机制（根据任务复杂度选择合适模型规模）。

Result: 实验结果显示OI-MAS持续优于基线多智能体系统，准确率提升高达12.88%，同时成本降低高达79.78%。

Conclusion: 通过自适应模型选择策略，OI-MAS框架在提升多智能体系统性能的同时显著降低了计算成本，为解决多智能体系统计算效率问题提供了有效方案。
...
### [280] [Key-Value Pair-Free Continual Learner via Task-Specific Prompt-Prototype](https://arxiv.org/abs/2601.04864)
*Haihua Luo,Xuming Ran,Zhengji Li,Huiyan Xue,Tingting Jiang,Jiangrong Shen,Tommi Kärkkäinen,Qi Xu,Fengyu Cong*

Main category: cs.AI

TL;DR: 提出了一种取消键值对的新连续学习方法Prompt-Prototype（ProP），通过任务特定提示和原型消除任务间干扰，提高可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于提示的连续学习方法依赖键值对，这会导致任务间干扰并限制可扩展性。需要一种能够消除这些限制的替代方法。

Method: 使用任务特定提示进行特征学习，同时使用原型捕获输入的典型特征。推理时通过绑定特定任务的提示和对应原型进行预测，并引入正则化约束来确保初始化稳定性。

Result: 在多个常用数据集上的实验证明了该方法的有效性，相比基于提示的主流方法获得了更好的结果。

Conclusion: 该方法取消了对键值对的依赖，为未来连续学习研究提供了新视角，有效解决了任务间干扰和可扩展性问题。
...
### [281] [Higher-Order Knowledge Representations for Agentic Scientific Reasoning](https://arxiv.org/abs/2601.04878)
...
### [282] [Precomputing Multi-Agent Path Replanning using Temporal Flexibility: A Case Study on the Dutch Railway Network](https://arxiv.org/abs/2601.04884)
...
### [283] [Flexible Manufacturing Systems Intralogistics: Dynamic Optimization of AGVs and Tool Sharing Using Coloured-Timed Petri Nets and Actor-Critic RL with Actions Masking](https://arxiv.org/abs/2601.04887)
*Sofiene Lassoued,Laxmikant Shrikant Bahetic,Nathalie Weiß-Borkowskib,Stefan Lierc,Andreas Schwunga*

Main category: cs.AI

TL;DR: 该研究提出了一种结合着色时间Petri网（CTPNs）与基于模型的演员-评论家强化学习（MBRL）的新方法，以解决柔性制造系统（FMS）中集成自动引导车辆（AGV）和刀具共享系统的复杂调度问题。该方法在标准基准测试中表现出色，同时大幅降低计算时间。


<details>
  <summary>Details</summary>
Motivation: 柔性制造系统在现代制造业中至关重要，传统的作业车间调度问题未能充分考虑自动引导车辆与刀具共享系统同时集成的复杂性。研究旨在解决这一多维挑战，提升FMS的适应性和效率。

Method: 采用着色时间Petri网提供形式化建模结构和动态动作屏蔽，以缩小动作搜索空间；结合基于模型的演员-评论家强化学习，增强对动态环境的适应性；融入前瞻策略优化AGV定位；使用小型公开基准和新开发的大型基准（受Taillard基准启发）进行评估；提供兼容gym的环境和实例生成器以确保可复现性；进行消融研究评估各组件贡献。

Result: 在小规模基准测试中与现有方法持平，在大规模基准测试中优于传统方法，显著缩短完工时间，同时计算时间减少十倍。消融研究证实了各框架组件的有效性。

Conclusion: 所提出的CTPNs与MBRL结合方法能有效解决FMS中集成AGV和刀具共享的复杂调度问题，在效率和计算时间方面均表现优异，为FMS调度提供了可扩展且高效的解决方案。
...
### [284] [DVD: A Robust Method for Detecting Variant Contamination in Large Language Model Evaluation](https://arxiv.org/abs/2601.04895)
...
### [285] [From Stories to Cities to Games: A Qualitative Evaluation of Behaviour Planning](https://arxiv.org/abs/2601.04911)
...
### [286] [Conversational AI for Rapid Scientific Prototyping: A Case Study on ESA's ELOPE Competition](https://arxiv.org/abs/2601.04920)
*Nils Einecke*

Main category: cs.AI

TL;DR: 本文通过欧洲航天局ELOPE竞赛案例，研究ChatGPT在科学快速原型开发中的作用，展示人机协作在科学竞争中的潜力与局限。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型作为科学编码伙伴的潜力，特别是在快速原型开发中如何加速科学发现，当前这方面研究仍不充分。

Method: 采用案例研究方法，通过在ELOPE竞赛中实际使用ChatGPT进行月球着陆器轨迹估计的原型开发，分析AI在代码生成、算法推理、数据处理和方法建议等方面的贡献。

Result: 尽管参赛较晚，团队获得第二名（得分0.01282），证明人机协作的有效性。ChatGPT在提供可执行代码、算法思路和数据处理方案方面表现出色，但也存在引入不必要结构变更、容易在讨论中混淆、偶尔产生关键错误等局限。

Conclusion: 对话式AI既能加速科学开发进程，又能支持概念性洞察。通过将LLMs结构化整合到科学工作流程中，并建立AI辅助科学研究的最佳实践，可以增强快速原型开发能力。
...
### [287] [T-Retriever: Tree-based Hierarchical Retrieval Augmented Generation for Textual Graphs](https://arxiv.org/abs/2601.04945)
*Chunyu Wei,Huaiyu Qin,Siyuan He,Yunhai Wang,Yueguo Chen*

Main category: cs.AI

TL;DR: T-Retriever引入树形检索方法，通过语义和结构引导的编码树解决传统图RAG在层次信息管理中的局限性


<details>
  <summary>Details</summary>
Motivation: 传统基于图的RAG方法在处理层次信息时面临两个关键限制：1）僵化的层次特定压缩配额会破坏局部图结构；2）优先考虑拓扑结构而忽略语义内容

Method: 提出了T-Retriever框架，将属性图检索重构为基于树的检索，使用语义和结构引导的编码树。关键创新包括自适应压缩编码和语义结构熵联合优化

Result: 在多种图推理基准测试中，T-Retriever显著优于当前最先进的RAG方法，为复杂查询提供更连贯且上下文相关的响应

Conclusion: T-Retriever通过结合结构连贯性和语义一致性的层次分区方法，有效增强了大型语言模型访问外部知识的能力
...
### [288] [ConMax: Confidence-Maximizing Compression for Efficient Chain-of-Thought Reasoning](https://arxiv.org/abs/2601.04973)
...
### [289] [AlgBench: To What Extent Do Large Reasoning Models Understand Algorithms?](https://arxiv.org/abs/2601.04996)
...
### [290] [An Empirical Investigation of Robustness in Large Language Models under Tabular Distortions](https://arxiv.org/abs/2601.05009)
*Avik Dutta,Harshit Nigam,Hosein Hasanbeig,Arjun Radhakrishna,Sumit Gulwani*

Main category: cs.AI

TL;DR: LLMs难以自动检测和校正表格数据的语义与结构扭曲，仅在明确提示下部分调整推理策略，准确率下降显著。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在表格数据发生语义和结构扭曲时的表现缺陷，揭示其缺乏自主校正能力，为未来研究提供方向。

Method: 引入专家标注的小型数据集，在表格问答任务中评估LLMs进行纠错和分析的能力，比较不同模型在扭曲条件下的表现。

Result: LLMs无法自动检测和校正表格扭曲，仅当明确提示时才部分校正但结果不稳定；即使是GPT-5.2等先进模型在扭曲下准确率也至少下降22%。

Conclusion: LLMs缺乏自主校正表格扭曲的内在能力，未来需研究模型如何像人类一样自主决定对齐表格输入，而非依赖显式提示或预处理。
...
### [291] [OptiSet: Unified Optimizing Set Selection and Ranking for Retrieval-Augmented Generation](https://arxiv.org/abs/2601.05027)
*Yi Jiang,Sendong Zhao,Jianbo Li,Bairui Hu,Yanrui Du,Haochun Wang,Bing Qin*

Main category: cs.AI

TL;DR: 提出了OptiSet框架，将集合选择与集合级排序统一起来，改善RAG中传统静态选择方法存在的冗余问题。


<details>
  <summary>Details</summary>
Motivation: 传统RAG方法依赖于静态选择最高相关性的k个段落，但这种方法无法利用段落间的组合增益，并经常引入大量冗余信息。

Method: 采用“扩展-精化”范式：首先将查询扩展为多个视角以获得多样化的候选池，然后通过重新选择形成紧凑的证据集；设计无强LLM监督的自合成策略，从生成器的条件效用变化中获得偏好标签；引入集合列表训练策略，联合优化集合选择和集合级排序。

Result: 大量实验表明，OptiSet在复杂组合问题上提高了性能，并使得生成更加高效。

Conclusion: OptiSet框架通过统一集合选择和集合级排序，有效地解决了传统RAG方法的局限性，提高了生成质量和效率。
...
### [292] [How to Set the Batch Size for Large-Scale Pre-training?](https://arxiv.org/abs/2601.05034)
...
### [293] [How to Set the Learning Rate for Large-Scale Pre-training?](https://arxiv.org/abs/2601.05049)
...
### [294] [Large language models can effectively convince people to believe conspiracies](https://arxiv.org/abs/2601.05050)
...
### [295] [Publishing FAIR and Machine-actionable Reviews in Materials Science: The Case for Symbolic Knowledge in Neuro-symbolic Artificial Intelligence](https://arxiv.org/abs/2601.05051)
...
### [296] [Learning Latent Action World Models In The Wild](https://arxiv.org/abs/2601.05230)
*Quentin Garrido,Tushar Nagarajan,Basile Terver,Nicolas Ballas,Yann LeCun,Michael Rabbat*

Main category: cs.AI

TL;DR: 为真实世界推理与规划，本文研究从无标签视频中学习潜在动作世界模型，提出连续有约束的潜在动作表征方案，能更好地应对多样化野外视频的复杂动作和噪音挑战，部分实现了动作的空间定位与跨视频可迁移性。控制器映射已知动作为潜在动作，使得潜在动作可作为通用接口解决规划任务，向真实世界扩展迈进了一步。


<details>
  <summary>Details</summary>
Motivation: 真实世界中的智能体需对自身动作的后果进行预测，世界模型虽然具备这种能力，但大多需要人工标注的动作标签，难以规模化获取。为此，需从视频中学习潜在动作模型，而现有研究多聚焦于简单的机器仿真、游戏或操作数据，缺乏针对多样化野外视频的研究。本文旨在从真实世界的野外视频中学习潜在动作世界模型，处理视频多样性、环境噪音、缺乏统一具身性等挑战，拓展潜在动作模型在真实世界中的应用范围。

Method: 提出了一套针对野外视频学习潜在动作世界模型的架构设计与评价方法框架。核心为引入连续但有约束的潜动作表征（对比常用的向量量化方法），使其能更丰富地捕捉复杂动作。同时，训练一个控制器，将已知动作映射到学得的潜在动作空间，形成通用接口，支持规划任务。在评估中，着重考察动作特性对视频变换的影响及在缺乏统一具身性条件下的空间局部化特性。

Result: 研究发现，连续有约束的潜动作模型能有效捕捉野外视频中复杂的动作信息，优于向量量化方法。例如，环境中源自智能体的变化（如人物进入房间）能在视频间迁移，表明学得的动作具有野外视频特异性。在缺乏跨视频统一具身性的条件下，学得的潜动作可在空间中相对摄像机关联地局部化。此外，控制器成功映射已知动作为潜在动作，使潜动作可作为通用的规划接口，在测试中表现出与带动作标签的基线方法相近的性能。

Conclusion: 本研究在将潜在动作模型拓展至真实世界方面迈出了重要一步。通过定义动作应满足的性质、设计相应的架构与评估方法，证明了连续有约束的潜在动作表征对野外视频复杂动作的建模有效性。尽管因缺少统一具身性，动作空间局部于摄像机，但通过控制器实现的动作映射与接口化，使得从无标签野外视频学得的世界模型能够有效支持规划任务，性能与基于动作标签的基准方法可比。
...
### [297] [Reinforced Efficient Reasoning via Semantically Diverse Exploration](https://arxiv.org/abs/2601.05053)
...
### [298] [Chain-of-Sanitized-Thoughts: Plugging PII Leakage in CoT of Large Reasoning Models](https://arxiv.org/abs/2601.05076)
*Arghyadeep Das,Sai Sreenivas Chintha,Rishiraj Girmal,Kinjal Pandey,Sharvi Endait*

Main category: cs.AI

TL;DR: 大语言模型（LRMs）的显式思维链（CoT）推理虽然提升性能，但也带来隐私泄露风险，本文提出了通过即时控制或微调实现隐私优先的推理方法。


<details>
  <summary>Details</summary>
Motivation: 现有的思维链推理虽然提升了解释性和可靠性，但中间推理过程常常泄露个人信息（PII），即使最终答案已脱敏，这使得模型存在严重的隐私风险，需要开发能实现隐私优先推理的可行干预方法。

Method: 提出了PII-CoT-Bench数据集，包含隐私敏感的思维链标注，以及覆盖现实和对抗场景的平衡评估基准；在能力不同的模型中分别采用基于提示的控制和微调两种方法进行隐私泄漏减少。

Result: 先进的模型通过提示控制效果最好，较弱的模型则需要微调才有显著的隐私泄露减少效果；两种方法都能够在各种模型和类别中大幅降低PII泄露，且对模型效用影响很小。

Conclusion: 私有思维链推理可以在几乎不损失效用的前提下实现，为构建隐私保护的推理系统提供了实用指导。
...
### [299] [Arabic Prompts with English Tools: A Benchmark](https://arxiv.org/abs/2601.05101)
*Konstantin Kubrak,Ahmed El-Moselhy,Ammar Alsulami,Remaz Altuwaim,Hassan Ismail Fawaz,Faisal Alsaby*

Main category: cs.AI

TL;DR: 提出了首个阿拉伯语工具调用基准，发现阿拉伯语交互时工具调用准确率下降5-10%


<details>
  <summary>Details</summary>
Motivation: 随着阿拉伯语原生大模型加速发展，评估框架滞后且多关注英语，缺乏对阿拉伯语工具调用性能的理解，特别是这些模型主要在英语数据上预训练，存在关键的研究空白

Method: 设计首个专门针对阿拉伯语的工具调用和智能体能力基准测试，建立标准化框架来评估阿拉伯语智能体工作流中的功能准确性和鲁棒性

Result: 发现显著性能差距：当用户使用阿拉伯语交互时，工具调用准确率平均下降5-10%，无论工具描述是阿拉伯语还是英语

Conclusion: 该基准揭示了关键挑战，旨在促进为阿拉伯语用户开发更可靠和语言公平的AI智能体
...
### [300] [Token-Level LLM Collaboration via FusionRoute](https://arxiv.org/abs/2601.05106)
...
### [301] [Controllable Memory Usage: Balancing Anchoring and Innovation in Long-Term Human-Agent Interaction](https://arxiv.org/abs/2601.05107)
...
### [302] [GlimpRouter: Efficient Collaborative Inference by Glimpsing One Token of Thoughts](https://arxiv.org/abs/2601.05110)
*Wenhao Zeng,Xuteng Zhang,Yuling Shi,Chao Hu,Yuting Chen,Beijun Shen,Xiaodong Gu*

Main category: cs.AI

TL;DR: 使用初始词符熵预测推理难度，提出训练自由的步骤协作框架以降低推理延迟同时保持精度


<details>
  <summary>Details</summary>
Motivation: 大推理模型生成多步推理链带来高推理延迟和计算成本，现有协作策略依赖局部词符概率或事后验证引入额外开销，需要更有效方法确定何时需要大模型能力或小模型效率

Method: 提出GlimpRouter框架：利用轻量模型生成每个推理步骤的第一个词符，基于初始词符熵预测步骤难度，当熵超过阈值时将步骤路由到大模型，实现免训练步骤级协作

Result: 在多项基准测试中显著降低推理延迟同时保持精度，在AIME25上比独立大模型提升10.7%精度同时减少25.9%推理延迟

Conclusion: 基于思维一瞥而非完整步骤评估来分配计算，为推理提供简单有效的协作机制，初始词符熵可作为推理步骤难度的强预测器
...
### [303] [Evaluative Fingerprints: Stable and Systematic Differences in LLM Evaluator Behavior](https://arxiv.org/abs/2601.05114)
*Wajid Nasser*
...
### [304] [Distilling the Thought, Watermarking the Answer: A Principle Semantic Guided Watermark for Large Reasoning Models](https://arxiv.org/abs/2601.05144)
...
### [305] [Observations and Remedies for Large Language Model Bias in Self-Consuming Performative Loop](https://arxiv.org/abs/2601.05184)
*Yaxuan Wang,Zhongteng Cai,Yujia Bao,Xueru Zhang,Yang Liu*

Main category: cs.AI

TL;DR: 本研究提出了自我消耗性能循环（SCPL）概念，探究了在受控性能反馈下合成数据在动态迭代训练过程中如何塑造偏见，发现性能循环会增加偏好偏见但减少差异偏见，并设计了一种基于奖励的拒绝采样策略来缓解偏见。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的快速发展导致使用合成数据训练未来模型的兴趣增加，但这会造成自我消耗的再训练循环——模型在自身输出上训练可能导致性能下降和偏见产生。现实应用中，先前部署的LLM可能影响其生成的数据，形成由用户反馈驱动的动态系统，例如如果模型持续忽略某用户群体，则该群体的查询数据收集会减少。受限于无法获取动态生产系统中的真实用户偏好数据，本研究旨在以受控方式隔离和分析反馈驱动的偏见演变。

Method: 引入自我消耗性能循环（SCPL）概念，在受控性能反馈环境下研究合成数据在动态迭代训练中的偏见形成。重点关注两种循环设置：典型的再训练设置和尚未充分探索的增量微调设置。在三个真实世界任务上进行实验，并设计了一种基于奖励的拒绝采样策略来缓解偏见。

Result: 实验发现，性能循环会增加偏好偏见（preference bias）但会减少差异偏见（disparate bias）。通过设计的奖励拒绝采样策略，能够有效缓解这些偏见，推动自我改进系统向更可信的方向发展。

Conclusion: 研究揭示了在自我消耗性能循环中，性能反馈会同时增加偏好偏见和减少差异偏见的双重效应。虽然这为偏见缓解提供了新视角，但仍需进一步研究如何平衡不同类型的偏见。提出的奖励拒绝采样策略为构建更可信的自我改进系统提供了可行方法，为未来研究动态迭代训练中的偏见演化机制奠定了基础。
...
### [306] [Internal Representations as Indicators of Hallucinations in Agent Tool Selection](https://arxiv.org/abs/2601.05214)
...
### [307] [MineNPC-Task: Task Suite for Memory-Aware Minecraft Agents](https://arxiv.org/abs/2601.05215)
*Tamil Sudaravan Mohan Doss,Michael Xu,Sudha Rao,Andrew D. Wilson,Balasaravanan Thoravi Kumaravel*

Main category: cs.AI

TL;DR: 提出了一个用户创建的基准测试框架MineNPC-Task，用于在开放世界Minecraft中评估具有记忆能力的交互式LLM智能体，包含参数化任务模板和机器可验证的检查机制。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM智能体评估方法往往依赖于合成提示，缺乏在真实开放世界环境中测试记忆感知和交互式智能体的系统化框架。

Method: 通过与专家玩家的协作玩法收集真实任务，将其规范化为带明确前置条件和依赖结构的参数化模板，设计机器可检查的验证器，并在有限知识策略下进行测试，禁止使用游戏世界外的捷径。

Result: 在GPT-4o上评估了8位有经验玩家的216个子任务，识别出代码执行、库存管理、引用和导航等方面的常见故障模式，同时观察到通过交互式澄清和轻量记忆支持的恢复能力。

Conclusion: 该框架为未来记忆感知嵌入式智能体提供透明、可复现的评估平台，参与者肯定了交互质量和界面可用性，同时指出需要提高任务间记忆的持久性。
